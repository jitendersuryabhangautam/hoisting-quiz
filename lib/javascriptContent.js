...
function normalizeAnswerParts(text) {
  return text
    .split(/\s*(?:\n|,)\s*/)
    .map((part) => part.trim())
  ...
  if (question.type === "output") {
    ...
    if ((question.expected ?? "").includes("\n")) {
      ...
      if (
        ...
        typedLines.every(
          (line, index) =>
            normalizeText(line) === normalizeText(expectedLines[index])
        )
      ) {
        return true;
      }

      const typedParts = normalizeAnswerParts(answer);
      const expectedParts = normalizeAnswerParts(question.expected ?? "");

      if (
        typedParts.length === expectedParts.length &&
        typedParts.every(
          (part, index) =>
            normalizeText(part) === normalizeText(expectedParts[index])
        )
      ) {
        return true;
      }
