...
process.on('SIGTERM', () => {
    ...
    hint: "Use `process.on('SIGINT')` for Ctrl+C as well.",
  },
  {
...
app.listen(3000);`,
    explanation:
      "`req.setTimeout` sets a timeout for the request; the callback can send an error response.",
    hint: "Also set `res.setTimeout` for response timeouts.",
  },
  {
...
// Run: air`,
  ...
  },
  {
    ...
    starter: `const express = require('express');
const app = express();
// your GET /users handler`,
    referenceSolution: `app.get('/users', (req, res) => {
...
app.post('/users', (req, res) => {
  ...
  },
  {
    ...
    explanation:
      "`req.params` contains route parameters. Always validate/convert types.",
    hint: "Use `parseInt` or `Number` for numeric IDs.",
  },
  {
    id: "query-parameters",
    type: "implementation",
...
app.post('/data', (req, res) => {
    ...
    explanation:
      "`express.json()` is built-in middleware that populates `req.body`.",
  ...
  },
  {
    ...
    starter: `function validateUser(req, res, next) {
    // your validation
}
app.post('/user', validateUser, (req, res) => { res.send('ok'); });`,
    ...
    explanation:
      "Validation middleware returns early with 400 if data is invalid, otherwise calls `next()`.",
    hint: "Use libraries like Joi or express-validator for complex schemas.",
  },
  {
...
console.log('Connected to PostgreSQL');`,
    explanation:
      "The `pg` client connects using a config object. Always close the connection when done.",
...
// your pool config`,
  ...
  },
  {
    ...
    starter: `const result = await pool.query('SELECT * FROM users');
// your code to handle result`,
  ...
  },
  {
    ...
    starter: `const text = 'INSERT INTO users(name, email) VALUES($1, $2) RETURNING *';
const values = ['Alice', 'alice@example.com'];
// your code`,
    referenceSolution: `const result = await pool.query(text, values);
...
console.log('Inserted:', newUser);`,
  ...
  },
  {
    ...
    starter: `const updateText = 'UPDATE users SET email = $1 WHERE id = $2 RETURNING *';
const updateValues = ['newemail@example.com', 1];
// your code`,
    referenceSolution: `const result = await pool.query(updateText, updateValues);
if (result.rowCount === 0) console.log('User not found');
  ...
  },
  {
    ...
    starter: `const deleteText = 'DELETE FROM users WHERE id = $1';
const deleteId = 1;
// your code`,
    referenceSolution: `const result = await pool.query(deleteText, [deleteId]);
...
if err == sql.ErrNoRows {
  ...
  },
  {
    ...
    starter: `const client = await pool.connect();
try {
    ...
    referenceSolution: `await client.query('BEGIN');
...
await client.query('COMMIT');`,
    explanation:
      "Transactions ensure atomicity. Always `COMMIT` on success, `ROLLBACK` on error.",
  ...
  },
  {
    ...
    explanation:
      "PostgreSQL error codes are numeric strings. `23505` is unique violation. Use them to differentiate errors.",
...
redis.on('connect', () => console.log('Connected to Redis'));`,
  ...
  },
  {
    ...
    starter: `import "github.com/go-redis/redis/v8"
// your code`,
    referenceSolution: `rdb := redis.NewClient(&redis.Options{
    Addr: "localhost:6379",
    Password: "",
    DB: 0,
})
...
if err != nil {
  ...
  },
  {
    ...
    starter: `// your redis command`,
    referenceSolution: `await redis.setex('session:123', 1800, 'user_data');`,
    explanation:
...
res.json({ url: result.Location });`,
  ...
  },
  {
    ...
    starter: `func uploadHandler(w http.ResponseWriter, r *http.Request) {
    // your code
}`,
    referenceSolution: `func uploadHandler(w http.ResponseWriter, r *http.Request) {
    ...
    explanation:
      "`ParseMultipartForm` parses the form; `FormFile` returns the first file. Save with `io.Copy`.",
    hint: "Limit memory usage with `ParseMultipartForm` argument.",
  },
  {
