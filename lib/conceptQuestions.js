...
\`;`,
  ...
  {
    id: "concept-micro-frontends",
    type: "concept",
    topic: "Architecture",
    title: "Micro‑frontends",
    prompt: "What are micro‑frontends and what problems do they solve?",
    expected:
      "Micro‑frontends extend microservices to the frontend, allowing independent teams to develop, deploy, and scale parts of the UI independently. They solve problems of monolithic frontends in large organisations: team scalability, technology diversity, independent deployments, and incremental upgrades.",
    keywords: [
      "micro-frontends",
      "architecture",
      "module federation",
    ...
    explanation:
      "Micro‑frontends are an architectural style where a frontend application is decomposed into smaller, independent pieces that can be developed, tested, and deployed by different teams. Each piece (micro‑frontend) owns a distinct part of the UI, such as a product listing, shopping cart, or checkout flow.\n\n**Problems solved:**\n- **Team scalability:** Multiple teams work independently without merging conflicts.\n- **Technology diversity:** Each micro‑frontend can use different frameworks (React, Vue, Angular).\n- **Independent deployments:** One team can deploy their part without coordinating with others.\n- **Incremental upgrades:** Rewrite legacy components gradually.\n- **Isolation:** A failure in one component doesn't crash the whole app.\n\n**Implementation approaches:**\n- **Build‑time integration:** Each micro‑frontend is published as a package; the host app includes them at build time. Simple but loses independent deployment.\n- **Run‑time integration via iframes:** Good isolation, but hard to share state and has UX issues.\n- **Run‑time integration via JavaScript (Webpack Module Federation):** Most powerful; loads remote components at runtime, shares dependencies.\n- **Run‑time integration via Web Components:** Framework‑agnostic, good for long‑term stability.\n- **Server‑side composition:** Edge‑side includes or server‑side fragments.\n\n**Challenges:**\n- **Cross‑cutting concerns:** Authentication, theming, analytics need coordination.\n- **Performance:** Multiple bundles may duplicate dependencies.\n- **Integration complexity:** CI/CD, routing, state sharing across boundaries.\n- **Testing:** End‑to‑end tests become more complex.\n\n**Tools:**\n- Webpack Module Federation (built‑in).\n- single‑spa (framework for orchestrating micro‑frontends).\n- Piral, Luigi, Open Components.\n\n**Interview tip:** Understand that micro‑frontends are not for every project; they add complexity. Be able to explain the trade‑offs and when they are appropriate (large organisations, multiple teams). Know the difference between build‑time and run‑time integration and the role of Module Federation in modern micro‑frontends.",
    code: "// Webpack Module Federation (host app – webpack.config.js)\nconst ModuleFederationPlugin = require('webpack/lib/container/ModuleFederationPlugin');\nmodule.exports = {\n  plugins: [\n    new ModuleFederationPlugin({\n      name: 'host',\n      remotes: {\n        checkout: 'checkout@http://localhost:3001/remoteEntry.js',\n        productList: 'productList@http://localhost:3002/remoteEntry.js',\n      },\n      shared: {\n        react: { singleton: true, eager: true, requiredVersion: '^18.0.0' },\n        'react-dom': { singleton: true, eager: true },\n      },\n    }),\n  ],\n};\n\n// Remote app (productList – webpack.config.js)\nnew ModuleFederationPlugin({\n  name: 'productList',\n  filename: 'remoteEntry.js',\n  exposes: {\n    './ProductList': './src/ProductList',\n  },\n  shared: { react: { singleton: true }, 'react-dom': { singleton: true } },\n});\n\n// Host consuming remote component\nimport React, { lazy, Suspense } from 'react';\nconst RemoteProductList = lazy(() => import('productList/ProductList'));\nfunction App() {\n  return (\n    <div>\n      <header>Header</header>\n      <Suspense fallback={<div>Loading products...</div>}>\n        <RemoteProductList />\n      </Suspense>\n      <footer>Footer</footer>\n    </div>\n  );\n}\n\n// single‑spa example (root config)\nimport { registerApplication, start } from 'single-spa';\nregisterApplication({\n  name: '@org/navbar',\n  app: () => System.import('@org/navbar'),\n  activeWhen: ['/'],\n});\nregisterApplication({\n  name: '@org/settings',\n  app: () => System.import('@org/settings'),\n  activeWhen: ['/settings'],\n});\nstart();\n\n// Cross‑app communication via custom events\ndocument.addEventListener('cart:updated', (event) => {\n  // update UI in other micro‑frontend\n});\n\n// or using a global shared state (Redux store exposed globally)\nwindow.__GLOBAL_STORE__ = createStore(reducer);\n\n// Server‑side composition with reverse proxy (Nginx)\nlocation /products {\n  proxy_pass http://product-service;\n}\nlocation /checkout {\n  proxy_pass http://checkout-service;\n}",
  },
  {
    id: "concept-webassembly",
    type: "concept",
    topic: "Performance",
    title: "WebAssembly (Wasm)",
    prompt: "What is WebAssembly and when should you use it?",
    expected:
      "WebAssembly is a low‑level, binary instruction format that runs in browsers at near‑native speed. It is ideal for computationally intensive tasks (video/audio processing, games, image editing, cryptography) where JavaScript performance is insufficient.",
    keywords: [
      "WebAssembly",
      "Wasm",
      "binary format",
    ...
    explanation:
      "WebAssembly (Wasm) is a portable, low‑level binary instruction format that runs in modern browsers alongside JavaScript. It is designed to be fast (near‑native execution) and secure (sandboxed). Wasm is not a replacement for JavaScript; rather, it is a compilation target for languages like C, C++, Rust, Go, and AssemblyScript.\n\n**How it works:**\n- Developers write code in a language like Rust or C++.\n- The code is compiled to `.wasm` binary.\n- The browser loads and instantiates the Wasm module.\n- JavaScript can call exported Wasm functions and pass data (numbers, linear memory).\n\n**When to use WebAssembly:**\n- **Performance‑critical tasks:** Video encoding/decoding, image processing, 3D rendering.\n- **Porting large existing C/C++ codebases:** Games (Unity, Unreal), CAD tools, image editors (GIMP, Photoshop), compilers.\n- **Cryptography, physics simulations, audio processing.**\n- **Data parsing (JSON, protobuf) at high speed.**\n\n**When NOT to use WebAssembly:**\n- DOM manipulation (Wasm cannot directly access the DOM; needs JS glue).\n- Simple logic where JavaScript is fast enough.\n- Projects that don't justify the added complexity.\n\n**Advantages:**\n- Near‑native performance.\n- Language‑agnostic (use C, Rust, Go, etc.).\n- Smaller binary size than equivalent asm.js.\n- Deterministic, easy to validate.\n\n**Disadvantages:**\n- Cannot directly access DOM, garbage collection, or browser APIs (needs JS interop).\n- Memory model (linear memory) is different from JS objects.\n- Tooling and debugging less mature.\n\n**Interview tip:** Understand that Wasm is a complement to JavaScript, not a replacement. Be able to give concrete use cases (Figma, Google Earth, AutoCAD Web, video editors). Know the basics of loading a Wasm module in JS and calling exported functions. Also mention the potential of WASI (WebAssembly System Interface) for server‑side Wasm.",
    code: "// Loading and using a WebAssembly module (Rust compiled)\n// Assume Fibonacci function exported\nasync function loadWasm() {\n  const response = await fetch('fib.wasm');\n  const bytes = await response.arrayBuffer();\n  const wasmModule = await WebAssembly.instantiate(bytes, {\n    // import objects (if any)\n  });\n  const { fib } = wasmModule.instance.exports;\n  console.log(fib(40)); // fast computation\n}\nloadWasm();\n\n// Using WebAssembly with streaming instantiation (more efficient)\nconst { instance } = await WebAssembly.instantiateStreaming(fetch('fib.wasm'));\nconsole.log(instance.exports.fib(40));\n\n// Passing memory between JS and Wasm\n// Assume Wasm exports a memory object and a function that writes to it\nconst memory = new WebAssembly.Memory({ initial: 10, maximum: 100 });\nconst { result } = await WebAssembly.instantiate(wasmBytes, {\n  env: { memory },\n});\n\n// Write data from JS into Wasm memory\nconst buffer = new Uint8Array(memory.buffer, 0, 100);\nbuffer.set([1,2,3,4,5]);\nresult.exports.processData(0, 5); // pass offset and length\n\n// Rust example (lib.rs)\n#[no_mangle]\npub extern \"C\" fn add(a: i32, b: i32) -> i32 {\n    a + b\n}\n\n// Compile: wasm-pack build --target web\n\n// Using AssemblyScript (TypeScript-like)\nexport function add(a: i32, b: i32): i32 {\n  return a + b;\n}\n\n// Emscripten example (C++ compiled to Wasm + JS glue)\n// #include <emscripten.h>\n// EMSCRIPTEN_KEEPALIVE\n// int multiply(int a, int b) { return a * b; }\n\n// Go example (tinygo)\n// //export add\n// func add(a, b int) int { return a + b }\n// func main() {}\n\n// Performance comparison (conceptual)\n// JavaScript: fib(40) ~ 1.5 seconds\n// WebAssembly: fib(40) ~ 0.3 seconds (5x faster)\n\n// Real use case: video processing with FFmpeg compiled to Wasm\nimport { createFFmpeg } from '@ffmpeg/ffmpeg';\nconst ffmpeg = createFFmpeg({ log: true });\nawait ffmpeg.load();\nffmpeg.FS('writeFile', 'input.mp4', videoData);\nawait ffmpeg.run('-i', 'input.mp4', 'output.gif');\nconst output = ffmpeg.FS('readFile', 'output.gif');\n\n// SharedArrayBuffer for multithreading in Wasm (requires cross‑origin isolation)\n// Add headers: Cross-Origin-Opener-Policy: same-origin, Cross-Origin-Embedder-Policy: require-corp",
  },
  {
    id: "concept-indexeddb",
    type: "concept",
    ...
    prompt: "What is IndexedDB and when would you use it?",
    expected:
      "IndexedDB is a low‑level, NoSQL database in the browser for storing large amounts of structured data, supporting indexes, transactions, and queries. It is used for offline‑first apps, caching complex data, and client‑side storage beyond localStorage limits.",
    keywords: [
      "IndexedDB",
      "client-side storage",
      "offline",
    ...
    explanation:
      "IndexedDB is a powerful client‑side storage API for storing large amounts of structured data (including files/blobs) in the browser. Unlike localStorage (limited to ~5‑10MB, synchronous), IndexedDB can store gigabytes, supports asynchronous operations, indexes for fast queries, and ACID transactions.\n\n**Key features:**\n- **Asynchronous API:** Non‑blocking, uses promises or callbacks.\n- **Key‑value store with indexes:** Store objects (any structured cloneable data) indexed by keys and custom indexes.\n- **Transactions:** ACID properties, with versioning for schema upgrades.\n- **Large capacity:** Typically >50MB, up to hundreds of MB or more (browser‑dependent).\n- **Binary data:** Supports Blobs, ArrayBuffers, and Files.\n\n**Common use cases:**\n- Offline‑first applications (Progressive Web Apps).\n- Caching API responses for offline access.\n- Client‑side databases for complex data (email clients, note‑taking apps).\n- File storage (images, documents).\n\n**Limitations:**\n- Verbose, callback‑heavy API (though modern wrappers like Dexie.js simplify it).\n- Not directly queryable with SQL; uses cursor‑based iteration.\n- Storage is per origin (domain), not shared across sites.\n- Asynchronous nature makes it more complex than localStorage.\n\n**Interview tip:** Be prepared to open a database, create an object store, add/get data, and use indexes. Also discuss when to use IndexedDB vs localStorage vs WebSQL (deprecated) and the role of service workers for offline caching.",
    code: "// Opening a database (request-based API)\nconst request = indexedDB.open('MyDatabase', 1);\n\nrequest.onerror = (event) => {\n  console.error('Database error:', event.target.error);\n};\n\nrequest.onsuccess = (event) => {\n  const db = event.target.result;\n  console.log('Database opened');\n  // use db\n};\n\nrequest.onupgradeneeded = (event) => {\n  const db = event.target.result;\n  // Create object store\n  const store = db.createObjectStore('users', { keyPath: 'id', autoIncrement: true });\n  // Create an index on the name field\n  store.createIndex('by_name', 'name', { unique: false });\n  store.createIndex('by_email', 'email', { unique: true });\n};\n\n// Adding data (within a transaction)\nconst db = request.result;\nconst transaction = db.transaction(['users'], 'readwrite');\nconst store = transaction.objectStore('users');\nconst user = { name: 'Alice', email: 'alice@example.com' };\nconst addRequest = store.add(user);\naddRequest.onsuccess = () => console.log('User added');\naddRequest.onerror = (e) => console.error('Error', e.target.error);\n\n// Getting data by key\nconst getRequest = store.get(1);\ngetRequest.onsuccess = () => console.log(getRequest.result);\n\n// Using an index to query by name\nconst index = store.index('by_name');\nconst nameRequest = index.get('Alice');\nnameRequest.onsuccess = () => console.log(nameRequest.result);\n\n// Cursor iteration\nconst cursorRequest = store.openCursor();\ncursorRequest.onsuccess = (event) => {\n  const cursor = event.target.result;\n  if (cursor) {\n    console.log('User:', cursor.value);\n    cursor.continue();\n  }\n};\n\n// Deleting data\nconst deleteRequest = store.delete(1);\n\n// Using Dexie.js (simpler Promise-based wrapper)\nimport Dexie from 'dexie';\nconst db = new Dexie('MyDatabase');\ndb.version(1).stores({\n  users: '++id, name, email'\n});\nawait db.users.add({ name: 'Bob', email: 'bob@example.com' });\nconst user = await db.users.get(1);\nconst alice = await db.users.where('name').equals('Alice').first();\n\n// Clearing database (delete all data)\nindexedDB.deleteDatabase('MyDatabase');\n\n// Using IndexedDB with service worker for offline caching\nself.addEventListener('fetch', (event) => {\n  event.respondWith(\n    caches.match(event.request).then(cached => cached || fetch(event.request))\n  );\n});\n// Store API responses in IndexedDB for offline use\n\n// Browser support check\nif ('indexedDB' in window) {\n  // supported\n}\n\n// Saving files/blobs\nconst blob = new Blob(['Hello'], { type: 'text/plain' });\nstore.add({ id: 'file1', content: blob });\n\n// Retrieving blob\nconst getBlob = store.get('file1');\ngetBlob.onsuccess = () => {\n  const blob = getBlob.result.content;\n  const url = URL.createObjectURL(blob);\n};\n\n// Transaction example (rollback on error)\ntry {\n  const tx = db.transaction(['users'], 'readwrite');\n  const store = tx.objectStore('users');\n  store.add({ id: 1, name: 'Alice' });\n  store.add({ id: 1, name: 'Bob' }); // duplicate key, error\n  // transaction will automatically rollback\n} catch (e) {\n  console.error('Transaction failed');\n}",
  },
  {
    id: "concept-nodejs-event-loop",
    type: "concept",
    ...
    prompt:
      "How does the Node.js event loop differ from the browser event loop?",
    expected:
      "Node.js event loop has more phases (timers, I/O, idle, poll, check, close) and is designed for server‑side I/O. The browser event loop prioritises rendering and user interactions, with microtasks and macrotasks. Node.js also has `process.nextTick` and `setImmediate`, which have no direct browser equivalents.",
    keywords: [
      "Node.js",
      "event loop",
      "libuv",
    ...
    explanation:
      "Both Node.js and browsers use event loops for asynchronous operations, but they have different implementations and priorities.\n\n**Browser event loop:**\n- Simplifies with macroTask queue (setTimeout, events) and microTask queue (Promise callbacks, MutationObserver).\n- Prioritises rendering (requestAnimationFrame) and user interactions.\n- After each macroTask, microtasks are drained, then possible rendering.\n\n**Node.js event loop (libuv):**\n- Has **six phases** that run in a fixed order:\n  1. **timers:** executes setTimeout and setInterval callbacks whose time has expired.\n  2. **pending callbacks:** executes I/O callbacks deferred to the next loop iteration.\n  3. **idle, prepare:** internal use.\n  4. **poll:** retrieves new I/O events; executes I/O callbacks (except timers, close, setImmediate).\n  5. **check:** executes setImmediate callbacks.\n  6. **close callbacks:** executes close event callbacks (e.g., socket.on('close')).\n- Between each phase, `process.nextTick` and microtasks are drained.\n\n**Key differences:**\n- **`setImmediate`:** Node.js specific; runs in the 'check' phase after I/O.\n- **`process.nextTick`:** Not part of the event loop phases; runs before the next phase after current operation completes, even before microtasks.\n- **Timers:** In browsers, setTimeout(..., 0) runs after microtasks; in Node.js, timer callbacks run in the timers phase, which may be delayed by poll phase.\n- **Performance:** Node.js event loop optimises for many concurrent I/O operations (e.g., thousands of connections).\n\n**Interview tip:** Understand the order: `process.nextTick` > microtasks (Promise) > timers (setTimeout) > setImmediate (in Node.js). Also know that `setImmediate` vs `setTimeout(...,0)` order is non‑deterministic in Node.js depending on phase when scheduled.",
    code: "// Node.js specific: nextTick vs microtask vs setImmediate vs setTimeout\nprocess.nextTick(() => console.log('nextTick 1'));\nPromise.resolve().then(() => console.log('promise 1'));\nsetTimeout(() => console.log('timeout 1'), 0);\nsetImmediate(() => console.log('immediate 1'));\n// Typical output order in Node.js:\n// nextTick 1\n// promise 1\n// timeout 1 (or immediate 1, depending on event loop state)\n// immediate 1 (or timeout 1)\n\n// Browser equivalent (no nextTick or setImmediate)\nPromise.resolve().then(() => console.log('promise'));\nsetTimeout(() => console.log('timeout'), 0);\n// Output: promise, timeout\n\n// Node.js event loop phases order demonstration\nconst fs = require('fs');\nfs.readFile(__filename, () => {\n  console.log('I/O callback (poll phase)');\n  setImmediate(() => console.log('setImmediate inside I/O'));\n  setTimeout(() => console.log('setTimeout inside I/O'), 0);\n  process.nextTick(() => console.log('nextTick inside I/O'));\n});\n// Output order:\n// I/O callback (poll phase)\n// nextTick inside I/O\n// setImmediate inside I/O\n// setTimeout inside I/O\n\n// nextTick recursion can starve the event loop\nfunction recursiveNextTick() {\n  process.nextTick(recursiveNextTick);\n}\n// recursiveNextTick(); // blocks I/O and timers\n\n// Using setImmediate for non‑blocking recursion\nfunction recursiveSetImmediate() {\n  setImmediate(recursiveSetImmediate);\n}\n// safer\n\n// setTimeout(...,0) vs setImmediate order non‑deterministic in main module\nsetTimeout(() => console.log('timeout'), 0);\nsetImmediate(() => console.log('immediate'));\n// order can be immediate, timeout (if poll phase has no pending I/O)\n\n// process.nextTick vs Promise microtask: nextTick runs first in Node.js\nprocess.nextTick(() => console.log('nextTick'));\nPromise.resolve().then(() => console.log('promise'));\n// nextTick then promise\n\n// In browsers, nextTick does not exist; microtasks run after each macrotask",
  },
  {
    id: "concept-express-middleware",
    type: "concept",
    ...
    prompt: "What is Express middleware and how does it work?",
    expected:
      "Express middleware functions that have access to the request (req), response (res), and the next middleware function (next). They can execute code, modify req/res, end the request cycle, or call next() to pass control to the next middleware.",
    keywords: ["Express", "middleware", "app.use", "next", "request pipeline"],
    explanation:
      "Express middleware are functions that run in the request‑response cycle, sitting between the incoming request and the final route handler. They are executed sequentially in the order they are registered.\n\n**Middleware function signature:**\n```\nfunction(req, res, next) { ... }\n```\n\n**What middleware can do:**\n- Execute any code.\n- Modify the request (req) or response (res) objects.\n- End the request‑response cycle (by sending a response).\n- Call the next middleware in the stack using `next()`.\n- If a middleware does not end the request nor call `next()`, the request will hang.\n\n**Types of middleware:**\n- **Application‑level:** `app.use()`, `app.METHOD()` – applies to all routes or specific HTTP methods.\n- **Router‑level:** Bound to an instance of `express.Router()`.\n- **Error‑handling:** Takes four arguments (err, req, res, next) – must be defined after all other middleware.\n- **Built‑in:** `express.json()`, `express.urlencoded()`, `express.static()`.\n- **Third‑party:** `cors`, `morgan`, `helmet`, etc.\n\n**Order matters:** Middleware registered first executes first. Error‑handling middleware must be last.\n\n**Interview tip:** Be able to write custom middleware for logging, authentication, request validation, and error handling. Understand the difference between `app.use()` and `app.METHOD()` (e.g., `app.get()`). Also know that calling `next('route')` skips to the next route handler (only in router middleware).",
    code: "// Basic Express application with middleware\nconst express = require('express');\nconst app = express();\n\n// 1. Application-level middleware (executes for every request)\napp.use((req, res, next) => {\n  console.log(`${req.method} ${req.url}`);\n  next(); // pass control to next middleware\n});\n\n// 2. Built-in middleware\napp.use(express.json()); // parses JSON bodies\napp.use(express.urlencoded({ extended: true })); // parses URL-encoded bodies\napp.use(express.static('public')); // serves static files\n\n// 3. Third-party middleware (example)\nconst morgan = require('morgan');\napp.use(morgan('combined'));\n\n// 4. Route-specific middleware\nconst authMiddleware = (req, res, next) => {\n  const token = req.headers.authorization;\n  if (!token) {\n    return res.status(401).json({ error: 'Unauthorized' });\n  }\n  // verify token...\n  req.user = { id: 1, name: 'Alice' };\n  next();\n};\napp.get('/profile', authMiddleware, (req, res) => {\n  res.json(req.user);\n});\n\n// 5. Middleware that modifies the request\napp.use((req, res, next) => {\n  req.requestTime = Date.now();\n  next();\n});\n\n// 6. Error-handling middleware (four arguments, must be last)\napp.use((err, req, res, next) => {\n  console.error(err.stack);\n  res.status(500).json({ error: 'Something went wrong!' });\n});\n\n// 7. Middleware that ends the request (no next())\napp.use((req, res, next) => {\n  if (req.path === '/blocked') {\n    res.status(403).send('Access Denied');\n  } else {\n    next();\n  }\n});\n\n// 8. Router-level middleware\nconst router = express.Router();\nrouter.use((req, res, next) => {\n  console.log('Router middleware');\n  next();\n});\nrouter.get('/users', (req, res) => res.json([{ name: 'Alice' }]));\napp.use('/api', router);\n\n// 9. Middleware with async operations (always handle errors with next)\napp.use(async (req, res, next) => {\n  try {\n    const data = await fetchData();\n    req.data = data;\n    next();\n  } catch (err) {\n    next(err); // pass to error handler\n  }\n});\n\n// 10. Skipping remaining middleware (calling next('route'))\napp.get('/user/:id', (req, res, next) => {\n  if (req.params.id === '0') next('route'); // skip to next route handler\n  else next();\n}, (req, res) => {\n  res.send('regular user');\n});\napp.get('/user/:id', (req, res) => {\n  res.send('special user');\n});\n\napp.listen(3000);",
  },
  {
    id: "concept-jwt-authentication",
  ...
  },
  {
    ...
    prompt: "What are common design patterns in JavaScript?",
    expected:
      "Common patterns include Module, Singleton, Observer (Event Emitter), Factory, Prototype, Decorator, and Revealing Module pattern. These provide reusable solutions to common software design problems.",
    keywords: [
      "design patterns",
      "module",
      "singleton",
  ...
  },
  {
    ...
    prompt: "What are synthetic events in React?",
    expected:
      "Synthetic events are cross‑browser wrappers around native events, providing a consistent API and performance optimisations (event pooling in older React versions). They bubble through the React tree, not the DOM tree.",
    keywords: [
      "synthetic event",
      "event pooling",
      "react",
  ...
  },
  {
    ...
    expected:
      "Use try/catch for synchronous code, .catch() for promises, error boundaries in React, centralized error logging, and graceful fallbacks for users. Distinguish between expected errors (validation) and unexpected (network failures).",
    keywords: [
      "error handling",
      "try-catch",
      "error boundary",
    ...
    explanation:
      "Effective error handling improves user experience and debuggability. In JavaScript and React, a combination of patterns ensures that errors are caught, logged, and presented gracefully.\n\n**JavaScript error handling:**\n- **Synchronous code:** Use `try/catch` blocks around code that may throw.\n- **Promises:** Chain `.catch()` or use `try/catch` with `async/await`.\n- **Global handlers:** `window.onerror` and `unhandledrejection` for uncaught errors.\n- **Custom error classes:** Extend `Error` to create domain‑specific errors (e.g., `ValidationError`, `NetworkError`).\n- **Error boundaries in React:** Catch render errors in component trees.\n\n**React error handling:**\n- **Error boundaries:** Class components with `componentDidCatch` and `getDerivedStateFromError`.\n- **Suspense boundaries:** Handle async data fetching errors (with React 18+).\n- **Event handler errors:** Use try/catch inside handlers and update state to show error UI.\n\n**Best practices:**\n1. **Distinguish error types:** Expected errors (validation, user input) vs unexpected (network, bugs).\n2. **Provide user feedback:** Show friendly error messages, not technical stack traces.\n3. **Log errors to a service:** Use Sentry, LogRocket, or custom endpoint.\n4. **Graceful degradation:** Fallback UI, retry buttons, or cached data.\n5. **Idempotent retries:** For network errors, retry with exponential backoff.\n6. **Never swallow errors silently:** Always handle or log.\n7. **Use finally for cleanup:** Close resources, loading states.\n\n**Interview tip:** Be ready to implement a global error handler, an error boundary, and explain how to differentiate between error types. Understand that `try/catch` does not catch async errors; you must use `catch` on promises or `try/catch` with `await`.",
    code: "// Synchronous try/catch\ntry {\n  riskyOperation();\n} catch (error) {\n  console.error('Caught error:', error);\n  showUserMessage('Something went wrong');\n}\n\n// Async/await with try/catch\nasync function fetchData() {\n  try {\n    const response = await fetch('/api/data');\n    if (!response.ok) throw new Error(`HTTP ${response.status}`);\n    return await response.json();\n  } catch (error) {\n    logError(error);\n    return { error: 'Failed to load data' };\n  }\n}\n\n// Promise catch\nfetch('/api/data')\n  .then(res => res.json())\n  .catch(err => console.error(err));\n\n// Global error handlers\nwindow.onerror = (message, source, lineno, colno, error) => {\n  console.log('Global error:', { message, source, lineno, colno, error });\n  logToService(error);\n};\nwindow.addEventListener('unhandledrejection', (event) => {\n  console.error('Unhandled promise rejection:', event.reason);\n  logToService(event.reason);\n});\n\n// Custom error class\nclass ValidationError extends Error {\n  constructor(message) {\n    super(message);\n    this.name = 'ValidationError';\n  }\n}\n\n// React error boundary\nclass ErrorBoundary extends React.Component {\n  state = { hasError: false, error: null };\n  static getDerivedStateFromError(error) {\n    return { hasError: true, error };\n  }\n  componentDidCatch(error, errorInfo) {\n    console.error('React error caught:', error, errorInfo);\n    logToService(error, errorInfo);\n  }\n  render() {\n    if (this.state.hasError) {\n      return this.props.fallback || <h1>Something went wrong</h1>;\n    }\n    return this.props.children;\n  }\n}\n\n// Usage\nfunction App() {\n  return (\n    <ErrorBoundary fallback={<div>App crashed!</div>}>\n      <MyComponent />\n    </ErrorBoundary>\n  );\n}\n\n// Error handling in React event handler\nfunction SubmitButton() {\n  const [error, setError] = useState(null);\n  const handleSubmit = async () => {\n    try {\n      await submitForm();\n    } catch (err) {\n      setError(err.message);\n    }\n  };\n  return (\n    <div>\n      <button onClick={handleSubmit}>Submit</button>\n      {error && <div className=\"error\">{error}</div>}\n    </div>\n  );\n}\n\n// Retry logic with exponential backoff\nasync function fetchWithRetry(url, retries = 3, delay = 1000) {\n  for (let i = 0; i < retries; i++) {\n    try {\n      const res = await fetch(url);\n      if (!res.ok) throw new Error(`HTTP ${res.status}`);\n      return await res.json();\n    } catch (err) {\n      if (i === retries - 1) throw err;\n      await new Promise(resolve => setTimeout(resolve, delay * Math.pow(2, i)));\n    }\n  }\n}\n\n// Distinguishing error types\nfunction validateUser(user) {\n  if (!user.name) throw new ValidationError('Name is required');\n  // ...\n}\n\ntry {\n  validateUser(incompleteUser);\n} catch (err) {\n  if (err instanceof ValidationError) {\n    showFormError(err.message);\n  } else {\n    logUnexpectedError(err);\n    showGenericError();\n  }\n}\n\n// Finally block for cleanup\nlet loading = true;\ntry {\n  const data = await fetchData();\n  // process data\n} catch (err) {\n  // handle error\n} finally {\n  loading = false; // always runs\n}\n\n// Centralised logging service\nfunction logToService(error, context = {}) {\n  const logEntry = {\n    message: error.message,\n    stack: error.stack,\n    timestamp: new Date().toISOString(),\n    url: window.location.href,\n    userAgent: navigator.userAgent,\n    ...context\n  };\n  fetch('/api/log-error', {\n    method: 'POST',\n    body: JSON.stringify(logEntry),\n    keepalive: true\n  }).catch(() => {}); // fail silently\n}\n\n// React Suspense with error boundary (React 18)\n// <ErrorBoundary fallback={<ErrorDisplay />}>\n//   <Suspense fallback={<Loading />}>\n//     <DataComponent />\n//   </Suspense>\n// </ErrorBoundary>",
  },
  {
    id: "concept-object-methods",
    type: "concept",
    topic: "JavaScript Core",
    title: "Object Methods: create, assign, freeze, seal",
    prompt:
      "What do Object.create, Object.assign, Object.freeze, and Object.seal do? Explain the differences.",
    expected:
      "Object.create creates a new object with a specified prototype. Object.assign copies enumerable properties from source objects to a target. Object.freeze prevents any changes to an object (no add, remove, or modify). Object.seal prevents adding or removing properties but allows modification of existing properties.",
    keywords: [
      "Object.create",
      "Object.assign",
      "Object.freeze",
    ...
    explanation:
      "These static methods on the Object constructor serve different purposes for object manipulation and protection.\n\n**Object.create(proto, propertiesObject):**\n- Creates a new object with the specified prototype object.\n- Allows setting property descriptors (optional).\n- Useful for prototypal inheritance without constructor functions.\n- The created object has the specified object as its prototype chain link.\n\n**Object.assign(target, ...sources):**\n- Copies all enumerable own properties from source objects to the target object.\n- Returns the modified target object.\n- Performs shallow copy (nested objects are copied by reference).\n- Commonly used to clone objects or merge defaults.\n\n**Object.freeze(obj):**\n- Prevents any changes to the object: cannot add, delete, or modify existing properties.\n- Also prevents changes to the prototype and configurability.\n- Returns the same object (not a copy).\n- In strict mode, attempts to modify will throw; in non‑strict, they silently fail.\n- Shallow freezing only; nested objects remain unfrozen.\n\n**Object.seal(obj):**\n- Prevents adding or removing properties, but allows modifying existing properties.\n- Also marks all existing properties as non‑configurable.\n- Returns the sealed object.\n- Shallow sealing only.\n\n**Comparison table:**\n| Method | Add props | Remove props | Modify props | Prototype change |\n|--------|-----------|--------------|--------------|------------------|\n| Object.freeze | No | No | No | No |\n| Object.seal | No | No | Yes | No |\n| Object.assign | N/A (target) | N/A (target) | Overwrites | N/A |\n| Object.create | N/A (new object) | N/A | N/A | Sets prototype |\n\n**Interview tip:** Understand that freeze and seal are shallow. Use `deepFreeze` for complete immutability (recursive). Object.assign is a convenient alternative to spread operator for objects. Object.create is the modern way to set up prototype chains instead of using `__proto__` or constructor functions.",
    code: "// Object.create\nconst parent = { greet: () => 'Hello' };\nconst child = Object.create(parent);\nchild.name = 'Alice';\nconsole.log(child.greet()); // 'Hello' (inherited)\nconsole.log(child.name);    // 'Alice' (own)\n\n// Object.assign\nconst target = { a: 1 };\nconst source = { b: 2, c: 3 };\nconst result = Object.assign(target, source);\nconsole.log(target); // { a: 1, b: 2, c: 3 }\nconsole.log(result === target); // true (same object)\n\n// Shallow copy with Object.assign\nconst original = { a: 1, nested: { b: 2 } };\nconst copy = Object.assign({}, original);\ncopy.nested.b = 99;\nconsole.log(original.nested.b); // 99 (nested is shared)\n\n// Object.freeze\nconst frozen = { a: 1, b: { c: 2 } };\nObject.freeze(frozen);\nfrozen.a = 100;        // ignored (or throws in strict mode)\nfrozen.c = 3;          // ignored\nfrozen.b.c = 200;      // allowed! (nested is not frozen)\nconsole.log(frozen);   // { a: 1, b: { c: 200 } }\n\n// Deep freeze (recursive)\nfunction deepFreeze(obj) {\n  Object.values(obj).forEach(val => {\n    if (val && typeof val === 'object') deepFreeze(val);\n  });\n  return Object.freeze(obj);\n}\nconst deepFrozen = deepFreeze({ a: 1, b: { c: 2 } });\n// deepFrozen.b.c = 99; // silently fails (or throws in strict mode)\n\n// Object.seal\nconst sealed = { a: 1, b: 2 };\nObject.seal(sealed);\nsealed.a = 100;        // works\nsealed.c = 3;          // ignored\ndelete sealed.b;       // ignored\nconsole.log(sealed);   // { a: 100, b: 2 }\n\n// Checking state\nconsole.log(Object.isFrozen(frozen));   // true\nconsole.log(Object.isSealed(sealed));   // true\nconsole.log(Object.isExtensible(sealed)); // false (sealed and frozen make non-extensible)\n\n// Object.assign with multiple sources\nconst defaults = { theme: 'light', showHeader: true };\nconst userPrefs = { theme: 'dark' };\nconst config = Object.assign({}, defaults, userPrefs);\nconsole.log(config); // { theme: 'dark', showHeader: true }\n\n// Object.create with property descriptors\nconst obj = Object.create(null, {\n  id: { value: 1, writable: true, enumerable: true, configurable: true },\n  name: { value: 'Alice', enumerable: true }\n});\n\n// Object.assign vs spread operator (equivalent for shallow clone)\nconst spreadClone = { ...original };\n\n// Performance note: Object.assign and spread have similar performance",
  },
  {
    ...
    prompt:
      "Explain the differences between map, filter, reduce, some, every, flat, and flatMap. When would you use each?",
    expected:
      "map transforms each element into a new array; filter selects elements; reduce accumulates values; some/every test conditions; flat flattens nested arrays; flatMap maps then flattens by one level. Use them for functional data transformation.",
    keywords: [
      "map",
      "filter",
      "reduce",
    ...
    explanation:
      "JavaScript arrays provide several higher‑order methods for functional programming. Understanding each is essential for writing declarative, readable code.\n\n**map()**\n- Creates a new array populated with results of calling a function on every element.\n- Use when you need to transform each element (e.g., extract a property, apply calculation).\n- Returns an array of the same length.\n\n**filter()**\n- Creates a new array with elements that pass a test function.\n- Use when you need to select a subset based on condition.\n- Returns an array of the same or shorter length.\n\n**reduce()**\n- Accumulates array values into a single result (e.g., sum, product, object, array).\n- Can also implement map/filter using reduce (but specialised methods are clearer).\n- Receives accumulator, current value, index, array.\n\n**some()**\n- Returns true if at least one element satisfies the condition.\n- Short‑circuits on the first true.\n- Use for existence checks (e.g., does array contain a number > 10?).\n\n**every()**\n- Returns true if all elements satisfy the condition.\n- Short‑circuits on first false.\n- Use for validation (e.g., are all items in stock?).\n\n**flat()**\n- Creates a new array with all sub‑array elements concatenated up to specified depth.\n- Default depth = 1.\n- Use to flatten nested arrays.\n\n**flatMap()**\n- Maps each element using a mapping function, then flattens the result by one level.\n- Equivalent to `map(...).flat(1)` but more efficient.\n- Use when you need to produce multiple output elements per input (e.g., splitting strings, expanding lists).\n\n**Interview tip:** Understand that these methods do not mutate the original array (except reduce can, but shouldn't). Be ready to chain them: `arr.filter(pred).map(transform).reduce(sum)`. Also know that `flatMap` is ideal for removing or adding elements during mapping.",
    code: "// map: transform each element\nconst numbers = [1, 2, 3];\nconst doubled = numbers.map(n => n * 2); // [2,4,6]\n\n// filter: select elements\nconst evens = numbers.filter(n => n % 2 === 0); // [2]\n\n// reduce: accumulate\nconst sum = numbers.reduce((acc, n) => acc + n, 0); // 6\nconst product = numbers.reduce((acc, n) => acc * n, 1); // 6\n// group by property\nconst people = [{ name: 'Alice', age: 30 }, { name: 'Bob', age: 25 }];\nconst ageSum = people.reduce((acc, p) => acc + p.age, 0); // 55\n\n// some: any matching\nconst hasEven = numbers.some(n => n % 2 === 0); // true\n\n// every: all matching\nconst allEven = numbers.every(n => n % 2 === 0); // false\n\n// flat: flatten nested arrays\nconst nested = [1, [2, [3, [4]]]];\nconsole.log(nested.flat());      // [1,2,[3,[4]]] (depth 1)\nconsole.log(nested.flat(2));     // [1,2,3,[4]]\nconsole.log(nested.flat(Infinity)); // [1,2,3,4]\n\n// flatMap: map then flatten one level\nconst sentences = ['Hello world', 'Goodbye world'];\nconst words = sentences.flatMap(s => s.split(' '));\nconsole.log(words); // ['Hello', 'world', 'Goodbye', 'world']\n\n// filter + map (inefficient for one pass, but clearer)\nconst positiveDoubled = numbers.filter(n => n > 0).map(n => n * 2);\n\n// reduce to implement map (not recommended, for understanding)\nconst mappedUsingReduce = numbers.reduce((acc, n) => {\n  acc.push(n * 2);\n  return acc;\n}, []);\n\n// real‑world example: extracting unique categories from products\nconst products = [\n  { name: 'Laptop', categories: ['electronics', 'computers'] },\n  { name: 'Shirt', categories: ['clothing'] },\n  { name: 'Mouse', categories: ['electronics'] }\n];\nconst allCategories = products.flatMap(p => p.categories);\nconst uniqueCategories = [...new Set(allCategories)];\nconsole.log(uniqueCategories); // ['electronics', 'computers', 'clothing']\n\n// some for validation\nconst cart = [\n  { productId: 1, quantity: 2, available: true },\n  { productId: 2, quantity: 1, available: false }\n];\nconst hasUnavailable = cart.some(item => !item.available); // true\n\n// every for validation\nconst allAvailable = cart.every(item => item.available); // false\n\n// reduce to build an object map\nconst users = [{ id: 1, name: 'Alice' }, { id: 2, name: 'Bob' }];\nconst userMap = users.reduce((map, user) => {\n  map[user.id] = user;\n  return map;\n}, {});\nconsole.log(userMap[2]); // { id: 2, name: 'Bob' }",
  },
  {
    id: "concept-for-in-for-of",
    type: "concept",
    topic: "JavaScript Core",
    title: "for...in vs for...of vs forEach",
    prompt:
      "What are the differences between for...in, for...of, and forEach? When should you use each?",
    expected:
      "for...in iterates over enumerable property keys (including prototype chain); for...of iterates over iterable values (arrays, strings, maps, sets); forEach is an array method that calls a function for each element. Use for...of for values, forEach for arrays with side effects, for...in for object keys (with hasOwnProperty check).",
    keywords: [
      "for...in",
      "for...of",
      "forEach",
    ...
    explanation:
      "JavaScript offers multiple ways to iterate, each with different behaviour and use cases.\n\n**for...in**\n- Iterates over **enumerable property names** (keys) of an object, including inherited properties from the prototype chain.\n- Use for debugging or when you need to iterate over object keys, but always combine with `hasOwnProperty` to exclude prototype properties.\n- Not recommended for arrays because it iterates over indices as strings and includes custom properties.\n\n**for...of**\n- Iterates over **iterable values** (strings, arrays, maps, sets, arguments, NodeLists).\n- Works with the iterator protocol (objects with `Symbol.iterator`).\n- Returns the values themselves, not indices.\n- Cannot directly iterate over plain objects (use `Object.keys`, `Object.values`, or `Object.entries`).\n\n**forEach**\n- Method on arrays (and some array‑like objects via `Array.from`).\n- Calls a callback function for each element.\n- Cannot break early (use `some`, `every`, or a `for` loop for breaking).\n- Does not return a new array (unlike `map`).\n- Cannot use `await` inside easily (use `for...of` with `async/await`).\n\n**Comparison table:**\n| Feature | for...in | for...of | forEach |\n|---------|----------|----------|---------|\n| Works on | Objects (keys) | Iterables (values) | Arrays |\n| Iterates over | Property names | Values | Values |\n| Includes prototype | Yes (use hasOwnProperty) | No (iterable protocol) | No |\n| Can break early | No (no break) | Yes (break, return) | No |\n| Async/await | Not directly | Yes | Awkward |\n| Returns value | N/A | N/A | undefined |\n\n**Interview tip:** Understand that `for...in` should not be used for arrays. For objects, prefer `Object.keys(obj).forEach(...)` or `for...of` with `Object.values(obj)`. Use `for...of` when you need `break` or `await` inside loops.",
    code: "// for...in on objects (requires hasOwnProperty)\nconst person = { name: 'Alice', age: 30 };\nperson.__proto__.debug = true; // inherited property\nfor (const key in person) {\n  if (person.hasOwnProperty(key)) {\n    console.log(key, person[key]); // name Alice, age 30\n  }\n}\n\n// for...in on arrays (not recommended)\nconst arr = ['a', 'b', 'c'];\narr.customProp = 'test';\nfor (const index in arr) {\n  console.log(index, arr[index]); // 0 'a', 1 'b', 2 'c', 'customProp' 'test' (surprise!)\n}\n\n// for...of on arrays (values)\nfor (const value of arr) {\n  console.log(value); // 'a', 'b', 'c'\n}\n\n// for...of on strings\nfor (const char of 'hello') {\n  console.log(char); // h,e,l,l,o\n}\n\n// for...of on Map\nconst map = new Map([['a', 1], ['b', 2]]);\nfor (const [key, value] of map) {\n  console.log(key, value);\n}\n\n// for...of on Set\nconst set = new Set([1, 2, 3]);\nfor (const num of set) console.log(num);\n\n// forEach on arrays\narr.forEach((value, index, array) => {\n  console.log(value);\n});\n// cannot break: must use try/catch or every/some\n\n// Breaking out of loops\n// for...of supports break\nfor (const v of arr) {\n  if (v === 'b') break;\n  console.log(v); // 'a'\n}\n\n// forEach cannot break normally; workaround with some/every\narr.some(v => {\n  if (v === 'b') return true; // stop\n  console.log(v);\n  return false;\n});\n\n// Async/await with for...of\nasync function processItems(items) {\n  for (const item of items) {\n    await process(item); // sequential\n  }\n}\n\n// Async with forEach (does NOT work as expected)\n// items.forEach(async (item) => { await process(item); }); // all run in parallel, not sequential\n\n// Iterating object keys/values with for...of using Object.entries\nconst obj = { a: 1, b: 2 };\nfor (const [key, value] of Object.entries(obj)) {\n  console.log(key, value);\n}\n\n// Iterating object values\nfor (const value of Object.values(obj)) {\n  console.log(value);\n}\n\n// When to use which:\n// - Use for...of when you need values and may break or await.\n// - Use forEach when you need array method chaining or are already in a functional style.\n// - Use for...in only when iterating over object keys with hasOwnProperty or when you intentionally want prototype properties.\n// - Use traditional for loop when performance is critical (e.g., huge arrays).",
  },
  {
    id: "concept-eval",
    type: "concept",
    topic: "JavaScript Core",
    title: "eval() and why to avoid it",
    prompt: "What is eval() and why is it considered dangerous?",
    expected:
      "eval() executes a string as JavaScript code. It is dangerous because it can execute arbitrary code (security risk), hurts performance (disables optimisations), and makes code harder to debug. There are almost always safer alternatives (JSON.parse, Function constructor, or proper logic).",
    keywords: ["eval", "security", "performance", "code injection", "XSS"],
    explanation:
      "`eval()` is a global function that evaluates a string as JavaScript code and executes it in the current scope. It has legitimate uses (e.g., evaluating expressions in a REPL, or legacy systems), but it is heavily discouraged in production.\n\n**Why eval is dangerous:**\n1. **Security (code injection):** If the string contains user input, an attacker can execute arbitrary code (XSS, data theft). Example: `eval(userInput)` could delete files or steal cookies.\n2. **Performance:** JavaScript engines cannot optimise code that includes `eval()` because the code can change at runtime. This disables many compiler optimisations.\n3. **Debugging difficulty:** Dynamically generated code is hard to debug, set breakpoints, or profile.\n4. **Scope pollution:** `eval` can access and modify local variables, making code unpredictable.\n5. **CSP (Content Security Policy) conflict:** Many sites block `eval` via `unsafe-eval` directive.\n\n**Safer alternatives:**\n- **Parsing JSON:** Use `JSON.parse()` instead of `eval('(' + jsonString + ')')`.\n- **Dynamic property access:** Use `obj[propertyName]` instead of eval.\n- **Function constructor:** `new Function('return ' + expression)()` is also dangerous.\n- **Proper arithmetic:** Use code logic, not eval for calculations.\n- **Template engines:** Use safe templating libraries instead of constructing HTML with eval.\n\n**When eval is (rarely) acceptable:**\n- REPL environments (Node.js repl, browser dev tools).\n- Mathematical expression evaluators with strict sanitisation and sandboxing.\n- Server‑side scripts that trust the input completely (e.g., internal CLI tools).\n\n**Interview tip:** Emphasise that `eval()` is almost never necessary in modern JavaScript. Even `setTimeout('code', 1000)` and `Function()` are similarly dangerous. Use `JSON.parse`, computed property names, or callbacks instead.",
    code: "// DANGEROUS: eval with user input\nconst userInput = 'alert(\"XSS\")';\neval(userInput); // would execute alert\n\n// Instead, use JSON.parse for JSON\nconst jsonString = '{\"name\":\"Alice\"}';\n// const obj = eval('(' + jsonString + ')'); // bad\nconst safeObj = JSON.parse(jsonString); // good\n\n// Dynamic property access\nconst prop = 'color';\nconst obj2 = { color: 'red' };\n// eval('obj2.' + prop); // bad\nconst value = obj2[prop]; // good\n\n// Arithmetic expression evaluation (case where eval is tempting)\nfunction evaluateExpression(expr) {\n  // eval(expr); // dangerous and unnecessary\n  // Better: use math parser library or implement simple parser\n  // For safe arithmetic, use Function with limited context?\n  return new Function('return ' + expr)(); // still dangerous\n}\n\n// Safe alternative for simple arithmetic: process manually\nfunction safeAdd(a, b) { return a + b; } // not dynamic\n\n// eval can modify local scope (bad)\nfunction testEval() {\n  let x = 1;\n  eval('x = 2');\n  console.log(x); // 2 – unexpected side effect\n}\n\n// Indirect eval (slightly different scope, still dangerous)\nconst indirectEval = eval;\nindirectEval('var y = 10'); // creates global variable\n\n// CSP: Content-Security-Policy header can block eval\n// \"default-src 'self'; script-src 'self' 'unsafe-eval'\" blocks eval\n\n// Proper way to compute something based on property name\nconst operations = {\n  add: (a,b) => a+b,\n  subtract: (a,b) => a-b,\n};\nfunction compute(op, a, b) {\n  return operations[op]?.(a,b) ?? NaN;\n}\n\n// Using Function constructor (similar risks)\nconst adder = new Function('a', 'b', 'return a + b');\nconsole.log(adder(2,3)); // 5 – still eval-like\n\n// Recommendation: Always avoid eval.\n// For dynamic code execution, consider Web Workers (sandboxed) or server-side execution.",
  },
  {
    id: "concept-new-operator",
    type: "concept",
    topic: "JavaScript Core",
    title: "new operator and constructor functions",
    prompt:
      "What does the 'new' operator do in JavaScript? How does it create an object from a constructor function?",
    expected:
      "The new operator creates an instance of a user-defined object type or built-in object that has a constructor function. It creates a new empty object, sets its prototype, binds 'this', executes the constructor, and returns the object.",
    keywords: ["new", "constructor", "instance", "prototype", "this binding"],
    explanation:
      "The `new` operator in JavaScript is used to create an instance of an object that has a constructor function. When you call a function with `new`, the following steps happen:\n\n1. **Creates a new empty object:** `{}`\n2. **Sets the prototype:** Links the new object's internal `[[Prototype]]` to the constructor function's `prototype` property.\n3. **Binds `this`:** The constructor function is called with `this` bound to the new object.\n4. **Executes the constructor:** The function body adds properties/methods to `this`.\n5. **Returns the object:** If the constructor does not explicitly return an object, `new` returns the newly created object. If the constructor returns a non‑`null` object, that object is returned instead (primitives are ignored).\n\n**What `new` does NOT do:**\n- It does not create a closure around the constructor's scope.\n- It does not automatically add methods to the prototype (you must do that explicitly).\n\n**Constructor function conventions:**\n- Name starts with a capital letter (e.g., `Person`, `Animal`).\n- Use `this` to assign properties.\n- Do not return anything (or return `this`).\n\n**Without `new`:**\n- Calling a constructor function without `new` will execute it as a normal function, polluting the global scope (or throwing in strict mode).\n\n**ES6 classes** are syntactic sugar over the prototype/constructor pattern. `new` works the same way.\n\n**Interview tip:** Be able to manually replicate the `new` operator behaviour using `Object.create`. Understand that arrow functions cannot be constructors (they don't have a `prototype` property). Explain the difference between `instanceof` and `new`.",
    code: "// Constructor function\nfunction Person(name, age) {\n  this.name = name;\n  this.age = age;\n  // implicit return this\n}\nPerson.prototype.greet = function() {\n  return `Hello, I'm ${this.name}`;\n};\nconst alice = new Person('Alice', 30);\nconsole.log(alice.name); // 'Alice'\nconsole.log(alice.greet()); // 'Hello, I\\'m Alice'\nconsole.log(alice instanceof Person); // true\n\n// What `new` does (manual implementation - not exactly the same but conceptual)\nfunction manualNew(Constructor, ...args) {\n  // 1. create new object with prototype of Constructor\n  const obj = Object.create(Constructor.prototype);\n  // 2. call Constructor with obj as this\n  const result = Constructor.apply(obj, args);\n  // 3. return obj if Constructor returned nothing or primitive\n  return result instanceof Object ? result : obj;\n}\nconst bob = manualNew(Person, 'Bob', 25);\nconsole.log(bob.name); // 'Bob'\n\n// Without `new`: `this` is global (or undefined in strict mode)\nfunction Car(model) {\n  this.model = model;\n}\nconst car = Car('Tesla');    // missing new\nconsole.log(car); // undefined\nconsole.log(global.model); // 'Tesla' (in Node) or window.model (browser) – pollutes global\n\n// In strict mode, it throws\nfunction StrictCar(model) {\n  'use strict';\n  this.model = model;\n}\n// const c = StrictCar('Tesla'); // TypeError: Cannot set property 'model' of undefined\n\n// ES6 class (similar behaviour)\nclass Animal {\n  constructor(name) { this.name = name; }\n  speak() { return `${this.name} speaks`; }\n}\nconst dog = new Animal('Rex');\n\n// Constructor returning an object (overrides default)\nfunction PersonWithReturn(name) {\n  this.name = name;\n  return { custom: true };\n}\nconst p = new PersonWithReturn('Alice');\nconsole.log(p.name); // undefined\nconsole.log(p.custom); // true\n\n// Constructor returning a primitive (ignored)\nfunction PrimitiveReturn() {\n  this.value = 1;\n  return 42;\n}\nconst pr = new PrimitiveReturn();\nconsole.log(pr.value); // 1 (primitive ignored, returns instance)\n\n// Checking if a function was called with new (using new.target)\nfunction CheckNew() {\n  if (!new.target) {\n    throw new Error('Must call with new');\n  }\n  // ...\n}\nconst c1 = new CheckNew(); // ok\n// const c2 = CheckNew(); // throws",
  },
  {
    id: "concept-instanceof-typeof",
    type: "concept",
    topic: "JavaScript Core",
    title: "instanceof vs typeof",
    prompt:
      "What is the difference between typeof and instanceof in JavaScript? When would you use each?",
    expected:
      "typeof returns a string indicating the primitive type (e.g., 'number', 'string', 'object'). instanceof checks if an object's prototype chain contains the prototype of a constructor, used for custom types and classes.",
    keywords: [
      "typeof",
      "instanceof",
      "type checking",
    ...
    explanation:
      "Both `typeof` and `instanceof` are operators for type checking, but they serve different purposes and have different behaviours.\n\n**typeof:**\n- Returns a string indicating the type of the operand.\n- Works with both primitives and objects, but has known quirks: `typeof null === 'object'`, `typeof function(){} === 'function'`.\n- Useful for checking primitive types (`string`, `number`, `boolean`, `undefined`, `symbol`, `bigint`).\n- Cannot distinguish between different object types (e.g., `{}`, `[]`, `new Date()` all return `'object'`).\n\n**instanceof:**\n- Checks whether the prototype property of a constructor appears anywhere in the prototype chain of an object.\n- Works only with objects (returns false for primitives).\n- Useful for checking custom class instances or built‑in objects (`arr instanceof Array`, `date instanceof Date`).\n- Can be fooled by different execution contexts (iframes, different window objects).\n\n**Comparison table:**\n| Feature | typeof | instanceof |\n|---------|--------|------------|\n| Operand type | Any value | Object (primitives return false) |\n| Returns | String | Boolean |\n| Null check | `typeof null === 'object'` (quirk) | `null instanceof Object === false` |\n| Array check | `typeof [] === 'object'` | `[] instanceof Array === true` |\n| Custom class | `typeof instance === 'object'` | `instance instanceof Class === true` |\n| Cross‑realm | Works (returns 'object') | May fail (different prototype) |\n\n**Best practices:**\n- Use `typeof` for primitives and function detection.\n- Use `instanceof` for checking object types, especially custom classes.\n- For arrays, prefer `Array.isArray()` over `instanceof Array` (more robust across frames).\n- For null, compare directly: `value === null`.\n\n**Interview tip:** Be aware of the `typeof null` quirk and how to check for null (`value === null`). Understand that `instanceof` checks the prototype chain, so it works for inheritance. Also know that `typeof` returns `'function'` for functions, which is useful for detecting callable objects.",
    code: "// typeof examples\nconsole.log(typeof 42);           // 'number'\nconsole.log(typeof 'hello');      // 'string'\nconsole.log(typeof true);         // 'boolean'\nconsole.log(typeof undefined);    // 'undefined'\nconsole.log(typeof Symbol());     // 'symbol'\nconsole.log(typeof 123n);         // 'bigint'\nconsole.log(typeof {});           // 'object'\nconsole.log(typeof []);           // 'object'\nconsole.log(typeof null);         // 'object' (quirk)\nconsole.log(typeof function(){}); // 'function'\n\n// instanceof examples\nconsole.log([] instanceof Array);        // true\nconsole.log([] instanceof Object);       // true (Array inherits from Object)\nconsole.log({} instanceof Object);       // true\nconsole.log(new Date() instanceof Date); // true\nconsole.log(/regex/ instanceof RegExp);  // true\n\n// instanceof with primitives (always false)\nconsole.log(123 instanceof Number);      // false\nconsole.log('abc' instanceof String);    // false\ntry {\n  console.log(null instanceof Object);    // false (no error)\n} catch(e) {}\n\n// Custom class\nclass Animal {}\nclass Dog extends Animal {}\nconst dog = new Dog();\nconsole.log(dog instanceof Dog);    // true\nconsole.log(dog instanceof Animal); // true\nconsole.log(dog instanceof Object); // true\n\n// Cross‑realm issue (iframes)\n// const iframe = document.createElement('iframe');\n// document.body.appendChild(iframe);\n// const iframeArray = iframe.contentWindow.Array;\n// const arr = new iframeArray(1,2,3);\n// console.log(arr instanceof Array); // false (different Array constructor)\n// console.log(Array.isArray(arr));   // true (safe)\n\n// Better null check\nconst value = null;\nif (value === null) console.log('null');\n\n// Better array check\nconsole.log(Array.isArray([])); // true\n\n// typeof for function detection\nfunction myFn() {}\nif (typeof myFn === 'function') {\n  myFn();\n}\n\n// Edge case: typeof NaN\nconsole.log(typeof NaN); // 'number' (NaN is a number)\n// use isNaN() or Number.isNaN() to detect NaN\n\n// instanceof with built‑in constructors that can be faked\nconst fakeArray = { __proto__: Array.prototype };\nconsole.log(fakeArray instanceof Array); // true (but not a real array)",
  },
  {
    id: "concept-nodejs-event-loop-phases",
    type: "concept",
    topic: "Node.js",
    title: "Node.js Event Loop Phases",
    prompt:
      "What are the phases of the Node.js event loop? Explain each phase and the order of execution.",
    expected:
      "The Node.js event loop (libuv) has six phases: timers (setTimeout/setInterval), pending callbacks (I/O callbacks deferred), idle/prepare (internal), poll (retrieve I/O), check (setImmediate), and close (close events). Between phases, microtasks (Promise, nextTick) are drained.",
    keywords: [
      "Node.js",
      "event loop",
      "libuv",
    ...
    explanation:
      "The Node.js event loop is implemented by libuv and consists of several phases. Each phase has a queue of callbacks. The loop iterates through these phases in order, processing callbacks until the queue is empty or a maximum number of callbacks has been processed.\n\n**Phases in order:**\n1. **timers:** Executes callbacks scheduled by `setTimeout()` and `setInterval()` whose timers have elapsed.\n2. **pending callbacks:** Executes I/O callbacks that were deferred to the next loop iteration (e.g., some system errors).\n3. **idle, prepare:** Used internally by libuv; not relevant for JavaScript code.\n4. **poll:** Retrieves new I/O events; executes I/O callbacks (except timers, close, or setImmediate). This phase also blocks to wait for new events if there are no pending tasks.\n5. **check:** Executes `setImmediate()` callbacks.\n6. **close callbacks:** Executes close event callbacks (e.g., `socket.on('close')`).\n\n**Important behaviours:**\n- Between phases, the event loop drains the microtask queue (`process.nextTick` first, then `Promise` microtasks).\n- `process.nextTick` is not part of any phase; it runs immediately after the current operation, before the next phase.\n- The poll phase can block if there are no pending I/O and no timers scheduled, waiting for incoming events.\n- `setImmediate` is executed in the check phase, which happens after the poll phase.\n\n**Interview tip:** Understand the order: `nextTick` > Promise microtasks > timers (setTimeout) > I/O callbacks > setImmediate > close events. Be able to explain why `setTimeout(...,0)` and `setImmediate` order can be non‑deterministic when called from the main module (because the event loop start phase may still be in the timers phase).",
    code: "// Demonstrating phase order\nconst fs = require('fs');\n\nconsole.log('Start');\n\nsetTimeout(() => console.log('setTimeout (timers phase)'), 0);\nsetImmediate(() => console.log('setImmediate (check phase)'));\n\nfs.readFile(__filename, () => {\n  console.log('I/O callback (poll phase)');\n  process.nextTick(() => console.log('nextTick inside I/O'));\n  setImmediate(() => console.log('setImmediate inside I/O'));\n  setTimeout(() => console.log('setTimeout inside I/O'), 0);\n});\n\nprocess.nextTick(() => console.log('nextTick (before next phase)'));\nPromise.resolve().then(() => console.log('Promise microtask'));\n\nconsole.log('End');\n\n// Typical order (varies slightly due to event loop start):\n// Start\n// End\n// nextTick (before next phase)\n// Promise microtask\n// setTimeout (timers phase) – may run before or after I/O depending on event loop state\n// setImmediate (check phase) – same\n// I/O callback (poll phase)\n// nextTick inside I/O (after I/O callback)\n// setImmediate inside I/O (check phase after poll)\n// setTimeout inside I/O (timers phase after I/O)\n\n// Explanation of non‑deterministic setTimeout vs setImmediate (in main module)\n// If the event loop enters the timers phase first, setTimeout runs before setImmediate.\n// If the loop enters the check phase first, setImmediate runs first.\n// In practice, when called from the main module, the order can vary.\n\n// Blocking poll phase (if no timers and no I/O pending)\n// The loop will block in poll waiting for new events.\n\n// Use setImmediate for callbacks that should run after I/O events, but before timers? Actually after poll.\n\n// nextTick starvation: recursively calling process.nextTick can block the event loop.\nfunction starve() {\n  process.nextTick(starve);\n}\n// starve(); // never reaches I/O or timers\n\n// setImmediate is safe for recursion\nfunction safe() {\n  setImmediate(safe);\n}\n// safe(); // allows other phases to run between iterations\n\n// Checking phase execution order with multiple setImmediate\nsetImmediate(() => console.log('Immediate 1'));\nsetImmediate(() => console.log('Immediate 2'));\n// Both will run in the same check phase, in order.\n\n// Timers with delay 0 – not exactly 0; minimum 1ms in Node.js\nsetTimeout(() => console.log('Timeout'), 0);\n// Event loop will still need to wait at least 1ms",
  },
  {
    id: "concept-setimmediate-nexttick",
    type: "concept",
    topic: "Node.js",
    ...
    expected:
      "setTimeout schedules a callback after a minimum delay (macrotask). process.nextTick schedules a callback to run immediately after the current operation, before any I/O or timers (microtask with highest priority). setImmediate schedules a callback in the check phase, after I/O events.",
    keywords: [
      "setImmediate",
      "process.nextTick",
      "setTimeout",
    ...
    explanation:
      "Node.js provides three primary ways to schedule asynchronous callbacks with different priorities and behaviours.\n\n**setTimeout(cb, delay):**\n- Schedules a callback to be executed after at least `delay` milliseconds.\n- Callbacks are placed in the **timers phase** of the event loop.\n- Minimum delay is 1ms (even if set to 0).\n- Priority: lower than `nextTick` and microtasks, but can be pre‑empted by I/O.\n\n**process.nextTick(cb):**\n- Schedules a callback to be executed **immediately after the current operation** completes, before the event loop continues to the next phase.\n- Not part of the event loop phases; runs between phases, after the current operation.\n- **Highest priority** among all async callbacks.\n- Can starve the event loop if called recursively.\n\n**setImmediate(cb):**\n- Schedules a callback to be executed in the **check phase** of the event loop, after I/O events (poll phase).\n- Priority: higher than timers with 0 delay? Actually order can be non‑deterministic, but `setImmediate` always runs after I/O callbacks and before timers if the loop is at the beginning.\n- Safer for recursion than `nextTick`.\n\n**Priority order (from highest to lowest):**\n1. `process.nextTick` (same phase, before any other async).\n2. Promise microtasks (`.then`, `catch`, `finally`).\n3. `setImmediate` (check phase).\n4. I/O callbacks (poll phase).\n5. `setTimeout`/`setInterval` (timers phase).\n\n**Interview tip:** Understand the use cases: `process.nextTick` for allowing code to run after the current call stack but before I/O (e.g., error handling, event emission). `setImmediate` for deferring work until after I/O. `setTimeout` for delaying work by a specified time. Avoid recursive `nextTick`; use `setImmediate` instead.",
    code: "// Basic examples\nsetTimeout(() => console.log('setTimeout (timers)'), 0);\nsetImmediate(() => console.log('setImmediate (check)'));\nprocess.nextTick(() => console.log('nextTick (highest)'));\nPromise.resolve().then(() => console.log('promise (microtask)'));\nconsole.log('synchronous');\n\n// Output order (deterministic for this pattern):\n// synchronous\n// nextTick (highest)\n// promise (microtask)\n// setTimeout or setImmediate (order may vary)\n\n// Order between setTimeout(...,0) and setImmediate in main module is non‑deterministic\n// Demonstrated with a simple script:\n// node -e \"setTimeout(()=>console.log('timeout'),0); setImmediate(()=>console.log('immediate'));\"\n\n// However, inside an I/O callback, setImmediate always runs before setTimeout\nconst fs = require('fs');\nfs.readFile(__filename, () => {\n  setTimeout(() => console.log('timeout inside I/O'), 0);\n  setImmediate(() => console.log('immediate inside I/O'));\n  // Output: immediate inside I/O, then timeout inside I/O\n});\n\n// Recursion with nextTick (dangerous – blocks event loop)\nfunction recursiveNextTick(count) {\n  if (count <= 0) return;\n  process.nextTick(() => recursiveNextTick(count - 1));\n}\n// recursiveNextTick(100000); // fine, but can still block if infinite\n\n// Recursion with setImmediate (safe, allows other events)\nfunction recursiveSetImmediate(count) {\n  if (count <= 0) return;\n  setImmediate(() => recursiveSetImmediate(count - 1));\n}\n\n// Use process.nextTick for API design: ensuring callbacks are async even when data is ready instantly\nclass MyEmitter extends EventEmitter {\n  trigger() {\n    process.nextTick(() => this.emit('event'));\n  }\n}\n\n// Using setTimeout with 0 for deferring to next tick (but nextTick is better for immediate deferral)\nsetTimeout(() => {\n  // do something after current stack\n}, 0);\n\n// Performance: nextTick is more efficient than setTimeout(0) because it doesn't involve the timer heap.\n\n// Dangerous: not using nextTick in validators could cause sync error\nfunction asyncValidator(data, callback) {\n  // Bad: might throw synchronously\n  // if (!data) throw new Error('invalid');\n  // Better: use nextTick\n  process.nextTick(() => {\n    if (!data) callback(new Error('invalid'));\n    else callback(null, true);\n  });\n}\n\n// setImmediate vs setTimeout(0) benchmark (conceptual – setImmediate runs faster)\n// In practice, setImmediate is optimised for immediate deferral after I/O.",
  },
  {
    id: "concept-nodejs-buffer-streams",
    type: "concept",
    topic: "Node.js",
    title: "Node.js Buffer and Streams",
    prompt:
      "What are Buffers and Streams in Node.js? How are they used for efficient I/O?",
    expected:
      "Buffer is a temporary storage for binary data (raw memory allocation). Streams are abstract interfaces for working with streaming data (Readable, Writable, Duplex, Transform), allowing processing of data piece by piece without loading everything into memory.",
    keywords: [
      "Buffer",
      "Stream",
      "Readable",
    ...
    explanation:
      "Node.js provides Buffer and Stream to handle binary data efficiently, especially for I/O operations like file reading, network communication, and data processing.\n\n**Buffer:**\n- A fixed‑size chunk of raw memory allocated outside the V8 heap.\n- Used to represent binary data (e.g., from files, network sockets, or cryptographic operations).\n- Methods: `Buffer.from()`, `Buffer.alloc()`, `buf.toString()`, `buf.writeUInt8()`.\n- Important for interacting with lower‑level APIs (e.g., `fs.read`, `zlib`, `crypto`).\n\n**Streams:**\n- Abstract interfaces for handling streaming data in chunks.\n- **Readable:** Source of data (e.g., `fs.createReadStream`, HTTP requests).\n- **Writable:** Destination for data (e.g., `fs.createWriteStream`, HTTP responses).\n- **Duplex:** Both readable and writable (e.g., TCP sockets).\n- **Transform:** Duplex streams that modify data as it passes (e.g., `zlib.createGzip`, `crypto.createCipher`).\n- **Piping:** `readable.pipe(writable)` connects a readable stream to a writable stream, automatically managing backpressure.\n\n**Backpressure:**\n- When a writable stream cannot process data as fast as it is received, the readable stream is notified to pause (backpressure), preventing memory exhaustion.\n\n**Benefits:**\n- **Memory efficiency:** Process large files without loading them entirely into memory.\n- **Time efficiency:** Start processing data as soon as it arrives (e.g., video streaming).\n- **Composability:** Pipe streams together to create processing pipelines.\n\n**Interview tip:** Understand the difference between `Buffer` (raw memory) and typical JavaScript strings. Know the different types of streams and how to implement a custom transform stream. Be able to explain backpressure and how `.pipe()` handles it automatically.",
    code: "// Buffer creation\nconst buf1 = Buffer.from('Hello');      // from string\nconst buf2 = Buffer.from([0x48, 0x65]); // from array of bytes\nconst buf3 = Buffer.alloc(10);          // zero‑filled buffer of size 10\n\n// Buffer manipulation\nconst buf = Buffer.from('Node.js');\nconsole.log(buf.length);        // 7\nconsole.log(buf[0]);            // 78 (ASCII 'N')\nconsole.log(buf.toString());    // 'Node.js'\nconsole.log(buf.toString('hex')); // hex representation\n\n// Readable stream (file)\nconst fs = require('fs');\nconst readable = fs.createReadStream('largefile.txt', { highWaterMark: 64 * 1024 }); // 64KB chunks\nreadable.on('data', (chunk) => {\n  console.log(`Received ${chunk.length} bytes`);\n});\nreadable.on('end', () => console.log('No more data'));\nreadable.on('error', (err) => console.error(err));\n\n// Writable stream (file)\nconst writable = fs.createWriteStream('output.txt');\nwritable.write('Hello\\n');\nwritable.write('World\\n');\nwritable.end('Done');\n\n// Pipe: copy file\nconst src = fs.createReadStream('source.txt');\nconst dest = fs.createWriteStream('destination.txt');\nsrc.pipe(dest);\n\n// Piping with backpressure handling (auto)\n// The .pipe() method automatically handles backpressure.\n\n// Transform stream (uppercase)\nconst { Transform } = require('stream');\nconst upperCaseTransform = new Transform({\n  transform(chunk, encoding, callback) {\n    this.push(chunk.toString().toUpperCase());\n    callback();\n  }\n});\nprocess.stdin.pipe(upperCaseTransform).pipe(process.stdout);\n\n// Duplex stream (socket) – both directions\nconst net = require('net');\nconst server = net.createServer((socket) => {\n  socket.write('Welcome\\n');\n  socket.pipe(socket); // echo\n});\nserver.listen(3000);\n\n// Stream events\nreadable.on('readable', () => {\n  let chunk;\n  while ((chunk = readable.read()) !== null) {\n    console.log(chunk.toString());\n  }\n});\n\n// Handling backpressure manually (without pipe)\nfunction writeData(writable, data, encoding, callback) {\n  if (!writable.write(data, encoding)) {\n    writable.once('drain', callback);\n  } else {\n    process.nextTick(callback);\n  }\n}\n\n// Custom writable stream\nconst { Writable } = require('stream');\nconst myWritable = new Writable({\n  write(chunk, encoding, callback) {\n    console.log(chunk.toString());\n    callback();\n  }\n});\n\n// Combining streams: pipeline (better error handling than pipe)\nconst { pipeline } = require('stream');\npipeline(\n  fs.createReadStream('input.txt'),\n  upperCaseTransform,\n  fs.createWriteStream('output.txt'),\n  (err) => {\n    if (err) console.error('Pipeline failed', err);\n    else console.log('Pipeline succeeded');\n  }\n);\n\n// Buffer performance: prefer Buffer.alloc() over new Buffer(size) (deprecated)\n// Avoid Buffer.from() with large strings – can be slow\n\n// Streams in HTTP\nconst http = require('http');\nhttp.createServer((req, res) => {\n  fs.createReadStream('index.html').pipe(res); // serves file efficiently\n});\n\n// Using `finished` to detect stream end/error\nconst { finished } = require('stream');\nconst rs = fs.createReadStream('file.txt');\nfinished(rs, (err) => {\n  if (err) console.error('Stream failed', err);\n  else console.log('Stream completed');\n});",
  },
  {
    id: "concept-react-usestate-usereducer",
    type: "concept",
    topic: "React",
    title: "useReducer vs useState",
    prompt:
      "What is the difference between useState and useReducer in React? When would you choose useReducer over useState?",
    expected:
      "useState is simpler for independent state variables; useReducer is better for complex state logic that involves multiple sub‑values or when the next state depends on the previous state in complex ways. useReducer also helps manage state transitions more predictably and is easier to test.",
    keywords: [
      "useState",
      "useReducer",
      "state management",
    ...
    explanation:
      "Both `useState` and `useReducer` are React hooks for managing component state, but they serve different needs.\n\n**useState:**\n- Takes an initial value and returns `[state, setState]`.\n- Simple, suitable for independent state variables (strings, numbers, booleans, simple objects).\n- Updates can be done directly (`setCount(count+1)`) or with a function for previous state (`setCount(prev => prev+1)`).\n\n**useReducer:**\n- Takes a reducer function `(state, action) => newState` and an initial state.\n- Returns `[state, dispatch]`.\n- Ideal for complex state logic involving multiple fields or when transitions depend on action types.\n- Makes state updates more predictable and testable (reducer is pure).\n- Useful for state that updates frequently or when next state depends on previous state in non‑trivial ways.\n\n**When to use `useReducer`:**\n- State has multiple sub‑values (e.g., form with many fields, a shopping cart).\n- Complex state transitions that depend on action types.\n- When you need to optimise performance for deep updates (passing `dispatch` down instead of multiple callbacks).\n- When you plan to share state logic across multiple components (reducer can be extracted).\n- When you want to use Redux‑like patterns but within a component.\n\n**When to stick with `useState`:**\n- Simple independent state variables (e.g., toggle, input value).\n- No complex transition logic.\n- Performance not critical.\n\n**Interview tip:** Understand that `useReducer` can be combined with `useContext` to create a lightweight Redux alternative. Also know that `useReducer` is advantageous for testing because the reducer is a pure function.",
    code: "// useState example (simple counter)\nfunction Counter() {\n  const [count, setCount] = useState(0);\n  return <button onClick={() => setCount(count + 1)}>Count: {count}</button>;\n}\n\n// useReducer example (same counter)\nconst initialState = { count: 0 };\nfunction counterReducer(state, action) {\n  switch (action.type) {\n    case 'increment': return { count: state.count + 1 };\n    case 'decrement': return { count: state.count - 1 };\n    default: return state;\n  }\n}\nfunction CounterReducer() {\n  const [state, dispatch] = useReducer(counterReducer, initialState);\n  return (\n    <>\n      <button onClick={() => dispatch({ type: 'decrement' })}>-</button>\n      <span>{state.count}</span>\n      <button onClick={() => dispatch({ type: 'increment' })}>+</button>\n    </>\n  );\n}\n\n// Complex state scenario: form with many fields\nconst formInitialState = { name: '', email: '', age: '', errors: {} };\nfunction formReducer(state, action) {\n  switch (action.type) {\n    case 'setField':\n      return { ...state, [action.field]: action.value };\n    case 'setErrors':\n      return { ...state, errors: action.errors };\n    case 'reset':\n      return formInitialState;\n    default:\n      return state;\n  }\n}\nfunction ComplexForm() {\n  const [state, dispatch] = useReducer(formReducer, formInitialState);\n  return (\n    <form>\n      <input value={state.name} onChange={e => dispatch({ type: 'setField', field: 'name', value: e.target.value })} />\n      {/* ... other fields */}\n    </form>\n  );\n}\n\n// useReducer with context (lightweight Redux)\nconst StateContext = React.createContext();\nconst DispatchContext = React.createContext();\nfunction AppProvider({ children }) {\n  const [state, dispatch] = useReducer(counterReducer, initialState);\n  return (\n    <StateContext.Provider value={state}>\n      <DispatchContext.Provider value={dispatch}>\n        {children}\n      </DispatchContext.Provider>\n    </StateContext.Provider>\n  );\n}\nfunction DeepChild() {\n  const state = useContext(StateContext);\n  const dispatch = useContext(DispatchContext);\n  return <button onClick={() => dispatch({ type: 'increment' })}>{state.count}</button>;\n}\n\n// Testing reducer (pure function)\ntest('counterReducer increments', () => {\n  expect(counterReducer({ count: 0 }, { type: 'increment' })).toEqual({ count: 1 });\n});\n\n// Performance: useReducer can avoid passing many callbacks\nfunction Parent() {\n  const [state, dispatch] = useReducer(reducer, init);\n  // Dispatch identity is stable; child can use dispatch without rerenders\n  return <ChildWithDispatch dispatch={dispatch} />;\n}\n\n// Using lazy initialisation with useReducer\nfunction init(initialCount) { return { count: initialCount }; }\nconst [state, dispatch] = useReducer(reducer, 0, init);",
  },
  {
    ...
    topic: "React",
    title:
      "React Concurrent Features (useTransition, useDeferredValue, Suspense)",
    prompt:
      "What are useTransition, useDeferredValue, and Suspense? How do they improve React rendering?",
    expected:
      "useTransition and useDeferredValue mark state updates as non‑urgent, allowing React to interrupt rendering for higher‑priority updates. Suspense lets components wait for async data before rendering, showing fallback UIs.",
    keywords: [
      "useTransition",
      "useDeferredValue",
      "Suspense",
    ...
    explanation:
      "React 18 introduced concurrent rendering, which allows React to prepare multiple versions of the UI at the same time. `useTransition`, `useDeferredValue`, and `Suspense` are APIs that leverage this capability to improve perceived performance.\n\n**Suspense:**\n- Enables components to wait for asynchronous operations (data fetching, code splitting) before rendering.\n- Wraps a component that may suspend and displays a fallback UI while waiting.\n- Suspense boundaries can be nested; React shows the nearest ancestor fallback.\n- Works with `React.lazy` for code splitting and with data fetching libraries that support Suspense (e.g., Relay, SWR, React Query with suspense mode).\n\n**useTransition:**\n- Marks state updates as non‑urgent (transitions).\n- Returns `[isPending, startTransition]`.\n- Updates inside `startTransition` can be interrupted by urgent updates (user clicks, typing).\n- Use when you want to keep the UI responsive during expensive re‑renders (e.g., filtering a large list, navigating between pages).\n\n**useDeferredValue:**\n- Returns a deferred version of a value that may be outdated (stale) until the urgent updates finish.\n- Similar to debouncing but built into React's scheduler (adaptive).\n- Use when you receive a value from the parent and want to defer its processing (e.g., search input results).\n\n**Difference between useTransition and useDeferredValue:**\n- `useTransition` wraps state updates (you control the updates).\n- `useDeferredValue` wraps a value; React decides when to update it.\n\n**Interview tip:** Understand that concurrent rendering is not faster; it prioritises user interactions. Be able to implement a search input with `useDeferredValue` and a slow list rendering. Know that Suspense for data fetching still requires a compatible library.",
    code: "// Suspense with React.lazy (code splitting)\nconst LazyComponent = React.lazy(() => import('./HeavyComponent'));\nfunction App() {\n  return (\n    <Suspense fallback={<div>Loading...</div>}>\n      <LazyComponent />\n    </Suspense>\n  );\n}\n\n// useTransition for slow filtering\nfunction FilterList({ items }) {\n  const [filter, setFilter] = useState('');\n  const [isPending, startTransition] = useTransition();\n  const [filteredItems, setFilteredItems] = useState(items);\n\n  const handleChange = (e) => {\n    const value = e.target.value;\n    setFilter(value); // urgent update\n    startTransition(() => {\n      // non‑urgent: filter large list\n      const filtered = items.filter(item => item.includes(value));\n      setFilteredItems(filtered);\n    });\n  };\n\n  return (\n    <div>\n      <input value={filter} onChange={handleChange} />\n      {isPending && <div>Updating list...</div>}\n      <List items={filteredItems} />\n    </div>\n  );\n}\n\n// useDeferredValue for search results\nfunction SearchResults({ query }) {\n  const deferredQuery = useDeferredValue(query);\n  const results = useMemo(() => searchApi(deferredQuery), [deferredQuery]);\n  const isStale = query !== deferredQuery;\n  return (\n    <div style={{ opacity: isStale ? 0.5 : 1 }}>\n      {results.map(r => <div key={r.id}>{r.name}</div>)}\n    </div>\n  );\n}\n\n// Nested Suspense boundaries\nfunction ProfilePage() {\n  return (\n    <Suspense fallback={<UserSkeleton />}>\n      <UserProfile />\n      <Suspense fallback={<PostSkeleton />}>\n        <UserPosts />\n      </Suspense>\n    </Suspense>\n  );\n}\n\n// Suspense with error boundary combined\n<ErrorBoundary fallback={<ErrorUI />}>\n  <Suspense fallback={<Loading />}>\n    <DataComponent />\n  </Suspense>\n</ErrorBoundary>\n\n// Using startTransition without isPending\nconst handleClick = () => {\n  startTransition(() => {\n    setState(newValue);\n  });\n};\n\n// useTransition with timeout (React 18)\n// startTransition can also receive a config object with timeoutMs (unstable)\n\n// Streaming SSR with Suspense (Next.js App Router)\n// In Next.js, page components are Server Components with Suspense for streaming.",
  },
  {
    id: "concept-ts-utility-types",
    type: "concept",
    topic: "TypeScript",
    title: "TypeScript Utility Types",
    prompt:
      "What are TypeScript utility types? Explain Partial, Required, Pick, Omit, Record, ReturnType, and Parameters.",
    expected:
      "Utility types are built‑in TypeScript mapped types that transform existing types: Partial makes all properties optional; Required makes all required; Pick selects a subset of keys; Omit excludes keys; Record creates a key‑value type; ReturnType extracts function return type; Parameters extracts parameter tuple.",
    keywords: [
      "Partial",
      "Required",
      "Pick",
    ...
    explanation:
      "TypeScript provides several built‑in utility types to facilitate common type transformations. They are especially useful in libraries, generic code, and when working with complex data shapes.\n\n**Partial<Type>:**\n- Constructs a type with all properties of `Type` set to optional.\n- Useful for update operations, default props, or partial configuration.\n\n**Required<Type>:**\n- Constructs a type with all properties of `Type` set to required (removes optional `?`).\n- Opposite of `Partial`.\n\n**Pick<Type, Keys>:**\n- Constructs a type by picking a set of properties `Keys` from `Type`.\n- Useful for creating subsets or restricting APIs.\n\n**Omit<Type, Keys>:**\n- Constructs a type by omitting a set of properties `Keys` from `Type`.\n- Opposite of `Pick`.\n\n**Record<Keys, Type>:**\n- Constructs an object type with keys of type `Keys` and values of type `Type`.\n- Useful for dictionaries, maps, or index signatures.\n\n**ReturnType<Type>:**\n- Extracts the return type of a function type.\n- Useful for inferring return types from complex functions.\n\n**Parameters<Type>:**\n- Extracts the parameter types of a function type as a tuple.\n- Useful for building higher‑order functions or wrapper libraries.\n\n**Other common utility types:**\n- `Readonly<Type>`: makes all properties read‑only.\n- `Exclude<UnionType, ExcludedMembers>`: removes types from a union.\n- `Extract<UnionType, ExtractedMembers>`: extracts types from a union.\n- `NonNullable<Type>`: removes `null` and `undefined` from a union.\n- `Awaited<Type>`: resolves the type of a Promise.\n\n**Interview tip:** Understand that utility types are implemented using mapped types and conditional types. Be able to write a custom utility type (e.g., `DeepPartial`). Know the difference between `Pick` and `Omit`, and when to use `Record` over index signatures (`[key: string]: Type`).",
    code: "// Partial\ninterface User {\n  id: number;\n  name: string;\n  email: string;\n}\nfunction updateUser(id: number, updates: Partial<User>) {\n  // updates can have any subset of User properties\n}\nupdateUser(1, { name: 'Alice' }); // valid\n\n// Required\ninterface Config {\n  url?: string;\n  timeout?: number;\n}\nconst requiredConfig: Required<Config> = { url: 'http://api', timeout: 5000 };\n\n// Pick\ntype UserPreview = Pick<User, 'id' | 'name'>;\nconst preview: UserPreview = { id: 1, name: 'Alice' };\n\n// Omit\ntype UserWithoutEmail = Omit<User, 'email'>;\nconst noEmail: UserWithoutEmail = { id: 1, name: 'Alice' };\n\n// Record\nconst rolePermissions: Record<'admin' | 'user', string[]> = {\n  admin: ['read', 'write', 'delete'],\n  user: ['read']\n};\n// Alternative to index signature: Record<string, number> vs { [key: string]: number }\n\n// ReturnType\nfunction fetchData(): Promise<{ data: User }> {\n  return Promise.resolve({ data: { id: 1, name: 'Alice', email: 'a@b.com' } });\n}\ntype FetchResult = ReturnType<typeof fetchData>; // Promise<{ data: User }>\n\n// Parameters\nfunction greet(name: string, age: number): string {\n  return `${name} is ${age}`;\n}\ntype GreetParams = Parameters<typeof greet>; // [string, number]\n\n// Readonly\nconst frozen: Readonly<User> = { id: 1, name: 'Alice', email: 'a@b.com' };\n// frozen.id = 2; // Error: cannot assign to read‑only property\n\n// Exclude / Extract\ntype Status = 'loading' | 'success' | 'error' | 'idle';\ntype NonError = Exclude<Status, 'error'>; // 'loading' | 'success' | 'idle'\ntype LoadingOrError = Extract<Status, 'loading' | 'error'>; // 'loading' | 'error'\n\n// NonNullable\ntype MaybeString = string | null | undefined;\ntype DefinitelyString = NonNullable<MaybeString>; // string\n\n// Awaited\ntype PromiseUser = Awaited<ReturnType<typeof fetchData>>; // { data: User }\n\n// Custom utility: DeepPartial\ntype DeepPartial<T> = {\n  [P in keyof T]?: T[P] extends object ? DeepPartial<T[P]> : T[P];\n};\ninterface Nested { a: { b: { c: number } } }\nconst deep: DeepPartial<Nested> = { a: { b: {} } }; // valid\n\n// Composing utilities\ntype AdminUser = Required<Pick<User, 'id' | 'name'>> & { role: 'admin' };",
  },
  {
    id: "concept-ts-satisfies-as-const",
    type: "concept",
    topic: "TypeScript",
    title: "TypeScript satisfies operator and as const",
    prompt:
      "What does the satisfies operator do? How does as const differ from satisfies?",
    expected:
      "satisfies checks that an expression matches a type without changing the inferred type. as const makes an object/array deeply read‑only and literal (widening prevention). satisfies preserves inference while validating against a type; as const locks the value.",
    keywords: [
      "satisfies",
      "as const",
      "const assertion",
    ...
    explanation:
      "Both `satisfies` (TypeScript 4.9) and `as const` are used to improve type safety and inference, but they serve different purposes.\n\n**as const (const assertion):**\n- Tells TypeScript to infer the narrowest possible type.\n- Makes object properties `readonly`, array literals `readonly` tuples, and literals become literal types (not widened to `string` or `number`).\n- Prevents mutation and preserves the exact literal value.\n- Useful for constants, enums, or configurations that should never change.\n\n**satisfies operator:**\n- Checks that an expression conforms to a given type, but does not change the inferred type.\n- Combines type safety with the ability to keep the most specific inferred type (e.g., you keep literal types while ensuring compatibility with a broader type).\n- Addresses the problem where type annotations lose information (e.g., `const color: 'red' | 'blue' = 'red'` infers `'red' | 'blue'` instead of the literal `'red'`).\n\n**Comparison:**\n| Feature | as const | satisfies |\n|---------|----------|-----------|\n| Changes inferred type | Yes (narrows to literal/readonly) | No (keeps inference) |\n| Validates against a type | No (only asserts const-ness) | Yes |\n| Use case | Prevent widening, create readonly constant | Validate shape while preserving specific type |\n\n**Interview tip:** Use `as const` for constants that should be treated as exact values. Use `satisfies` instead of type annotations to avoid losing autocomplete and literal types, especially when working with object literals or component props.",
    code: "// as const example\nconst colors = ['red', 'green', 'blue'] as const;\n// inferred type: readonly ['red', 'green', 'blue']\n// colors.push('yellow'); // Error: push does not exist on readonly tuple\n\nconst config = { theme: 'dark', version: 1 } as const;\n// config.theme = 'light'; // Error (readonly)\n// type of config.theme is 'dark' (literal), not string\n\n// without as const – widens to string\nconst colorsWide = ['red', 'green', 'blue']; // string[]\n\n// as const with object literal\ntype Role = 'admin' | 'user';\nconst person = { name: 'Alice', role: 'admin' } as const;\n// person.role is 'admin' (literal), not Role.\n// But we might want to enforce that role matches Role while keeping literal.\n\n// satisfies example: validate that object conforms to a type while keeping autocomplete\nconst palette = {\n  primary: '#ff0000',\n  secondary: '#00ff00',\n  accent: '#0000ff'\n} satisfies Record<string, string>; // valid\n// palette.primary is '#ff0000' (literal), not string\n\n// Without satisfies – type annotation loses specificity\nconst palette2: Record<string, string> = { primary: '#ff0000' };\n// palette2.primary is string (too broad)\n\n// Using satisfies with union type\nconst eventMap = {\n  click: 'onClick',\n  hover: 'onHover'\n} satisfies Record<string, string>;\n// eventMap.click is 'onClick' literal, not just string\n\n// Combining both: as const for the value, satisfies for the shape\nconst apiRoutes = {\n  users: '/api/users',\n  posts: '/api/posts'\n} as const satisfies Record<string, `/${string}`>;\n// apiRoutes.users is '/api/users' (literal) and satisfies the pattern.\n\n// satisfies with function argument constraint\nfunction setConfig<T>(config: T & { version: number }) {\n  // ...\n}\nconst myConfig = { version: 1, name: 'app' } satisfies { version: number };\nsetConfig(myConfig); // works, myConfig still has literal 'app' as name.\n\n// Practical: React prop validation with satisfies\ntype ButtonProps = { variant: 'primary' | 'secondary'; size: 'sm' | 'lg' };\nconst defaultProps = {\n  variant: 'primary',\n  size: 'lg'\n} satisfies ButtonProps;\n// Now defaultProps.variant is 'primary', not 'primary' | 'secondary'.\n// Autocomplete works, and if we add wrong key, TypeScript errors.",
  },
  {
    id: "concept-ts-type-guards",
    type: "concept",
    topic: "TypeScript",
    title: "Type Guards (is, asserts, discriminated unions)",
    prompt:
      "What are type guards in TypeScript? Explain user‑defined type guards (is), assertion functions (asserts), and discriminated unions.",
    expected:
      "Type guards are expressions that perform runtime checks to narrow types. User‑defined type guards use `is`; assertion functions use `asserts`; discriminated unions use a common literal property to differentiate union members.",
    keywords: [
      "type guard",
      "is",
      "asserts",
    ...
    explanation:
      "TypeScript needs to narrow types at runtime. Type guards are ways to inform the compiler about the actual type of a variable within a scope.\n\n**Built‑in type guards:**\n- `typeof` (for primitives).\n- `instanceof` (for classes).\n- `in` (check property existence).\n\n**User‑defined type guards (is):**\n- Function that returns `value is Type`.\n- Tells TypeScript that if the function returns true, the argument is of that type.\n- Allows custom runtime checks.\n\n**Assertion functions (asserts):**\n- `function assert(condition: any): asserts condition` – throws if condition is falsy.\n- `function assertIsString(value: unknown): asserts value is string`.\n- Useful for validation and narrowing without returning a boolean.\n\n**Discriminated unions:**\n- Union of types that share a common literal property (the discriminator).\n- TypeScript narrows the union based on the discriminator value.\n- No function call needed; works with `if`/`switch` statements.\n\n**Interview tip:** Use `is` when you need to return a boolean and preserve narrowing. Use `asserts` when you want to throw on failure (e.g., validation functions). Use discriminated unions for algebraic data types.",
    code: "// Built‑in type guards\nfunction processValue(value: string | number) {\n  if (typeof value === 'string') {\n    console.log(value.toUpperCase()); // narrowed to string\n  } else {\n    console.log(value.toFixed(2));   // narrowed to number\n  }\n}\n\n// User‑defined type guard (is)\ninterface Cat { meow(): void; }\ninterface Dog { bark(): void; }\nfunction isCat(animal: Cat | Dog): animal is Cat {\n  return (animal as Cat).meow !== undefined;\n}\nfunction makeSound(animal: Cat | Dog) {\n  if (isCat(animal)) {\n    animal.meow(); // animal is Cat\n  } else {\n    animal.bark(); // animal is Dog\n  }\n}\n\n// Type guard for specific shape\ninterface User { name: string; age: number }\nfunction isUser(obj: any): obj is User {\n  return obj && typeof obj.name === 'string' && typeof obj.age === 'number';\n}\n\n// Assertion function (asserts)\nfunction assertIsString(value: unknown): asserts value is string {\n  if (typeof value !== 'string') {\n    throw new Error(`Expected string, got ${typeof value}`);\n  }\n}\nfunction useString(value: unknown) {\n  assertIsString(value);\n  console.log(value.toUpperCase()); // value is string after assertion\n}\n\n// Another assertion: assertTruthy\nfunction assertTruthy<T>(value: T): asserts value is NonNullable<T> {\n  if (!value) throw new Error('Value is falsy');\n}\n\n// Discriminated union\ntype Shape =\n  | { kind: 'circle'; radius: number }\n  | { kind: 'square'; sideLength: number }\n  | { kind: 'rectangle'; width: number; height: number };\nfunction getArea(shape: Shape): number {\n  switch (shape.kind) { // discriminator\n    case 'circle':\n      return Math.PI * shape.radius ** 2;\n    case 'square':\n      return shape.sideLength ** 2;\n    case 'rectangle':\n      return shape.width * shape.height;\n  }\n}\n\n// Discriminated union with exhaustive check (never)\nfunction assertNever(x: never): never {\n  throw new Error('Unexpected object: ' + x);\n}\nfunction getAreaExhaustive(shape: Shape): number {\n  switch (shape.kind) {\n    case 'circle': return Math.PI * shape.radius ** 2;\n    case 'square': return shape.sideLength ** 2;\n    case 'rectangle': return shape.width * shape.height;\n    default: return assertNever(shape); // ensures all cases covered\n  }\n}\n\n// Combining type guard with generic\nfunction hasProperty<K extends string>(obj: any, prop: K): obj is { [P in K]: unknown } {\n  return obj && prop in obj;\n}\n\n// Using in operator as type guard\nif ('radius' in shape) {\n  // shape is narrowed to type containing radius (circle?)\n}\n\n// Practical: API response guard\ninterface SuccessResponse { success: true; data: any }\ninterface ErrorResponse { success: false; error: string }\ntype ApiResponse = SuccessResponse | ErrorResponse;\nfunction handleResponse(res: ApiResponse) {\n  if (res.success) {\n    console.log(res.data); // narrowed\n  } else {\n    console.error(res.error); // narrowed\n  }\n}",
  },
  {
    id: "concept-jest-mocking",
    type: "concept",
    topic: "Testing",
    title: "Jest Mocking (jest.fn, jest.spyOn, jest.mock)",
    prompt:
      "How do you mock functions and modules in Jest? Explain jest.fn(), jest.spyOn(), and jest.mock().",
    expected:
      "jest.fn() creates a mock function; jest.spyOn() wraps an existing method to track calls; jest.mock() automatically mocks an entire module. They are used to isolate code under test, verify interactions, and replace dependencies.",
    keywords: [
      "jest.fn",
      "jest.spyOn",
      "jest.mock",
    ...
    explanation:
      "Jest provides powerful mocking capabilities to replace dependencies with controlled implementations during testing. This allows you to isolate the unit under test and verify how it interacts with its dependencies.\n\n**jest.fn():**\n- Creates a new mock function with no implementation.\n- You can set up return values, implementations, or track calls.\n- Methods: `mockReturnValue`, `mockResolvedValue`, `mockImplementation`, `mockRejectedValue`.\n- Useful for callback functions or simple dependencies.\n\n**jest.spyOn():**\n- Wraps an existing method (object method or module export) to track calls.\n- Can also mock the implementation while preserving the original.\n- `jest.spyOn(object, methodName).mockImplementation(...)`.\n- Use `mockRestore()` to restore original implementation.\n\n**jest.mock():**\n- Automatically mocks an entire module at the top level (hoisted).\n- Can provide a manual mock (factory) or use automatic mock (auto‑mocked module).\n- Useful for mocking entire libraries or modules to avoid real network/file system calls.\n\n**Matchers for mocks:**\n- `expect(mock).toHaveBeenCalled()`\n- `expect(mock).toHaveBeenCalledTimes(n)`\n- `expect(mock).toHaveBeenCalledWith(arg1, arg2)`\n- `expect(mock).toHaveBeenLastCalledWith(...)`\n\n**Interview tip:** Understand the difference between `jest.fn()` (for creating new mocks) and `jest.spyOn()` (for spying on existing methods). Know that `jest.mock` is hoisted, so it runs before imports. Understand how to mock ES modules (default vs named exports). Also know about `jest.clearAllMocks()`, `jest.resetAllMocks()`, and `jest.restoreAllMocks()` for cleanup.",
    code: "// jest.fn() examples\nconst mockFn = jest.fn();\nmockFn('hello');\nexpect(mockFn).toHaveBeenCalledWith('hello');\nexpect(mockFn).toHaveBeenCalledTimes(1);\n\n// Mock return value\nmockFn.mockReturnValue(42);\nconsole.log(mockFn()); // 42\n\n// Mock implementation\nmockFn.mockImplementation(x => x * 2);\nconsole.log(mockFn(5)); // 10\n\n// Mock resolved value (Promise)\nmockFn.mockResolvedValue('data');\nawait expect(mockFn()).resolves.toBe('data');\n\n// jest.spyOn() example\nconst math = { sum: (a, b) => a + b };\nconst sumSpy = jest.spyOn(math, 'sum');\nconst result = math.sum(2, 3);\nexpect(sumSpy).toHaveBeenCalledWith(2, 3);\nsumSpy.mockRestore(); // restore original\n\n// jest.spyOn with mockImplementation\njest.spyOn(math, 'sum').mockImplementation((a, b) => a * b);\nconsole.log(math.sum(2, 3)); // 6\n\n// jest.mock for module (hoisted)\n// __tests__/api.test.js\njest.mock('../api'); // auto‑mock\nimport { fetchUser } from '../api';\n// fetchUser is now a mock function\nfetchUser.mockResolvedValue({ name: 'Alice' });\n\n// Manual mock module (factory)\njest.mock('../api', () => ({\n  fetchUser: jest.fn().mockResolvedValue({ name: 'Mocked' }),\n  saveUser: jest.fn()\n}));\n\n// Mocking named exports in ES modules\njest.mock('../utils', () => ({\n  formatName: jest.fn(name => `Hello, ${name}`)\n}));\n\n// Mocking default export\njest.mock('../logger', () => ({\n  __esModule: true,\n  default: jest.fn()\n}));\n\n// Clearing and resetting mocks\nbeforeEach(() => {\n  jest.clearAllMocks(); // clears call counts, not implementations\n  jest.resetAllMocks(); // clears call counts and removes implementations\n  jest.restoreAllMocks(); // restores original implementations (for spies)\n});\n\n// Mock timers\njest.useFakeTimers();\nsetTimeout(() => console.log('timer'), 1000);\njest.advanceTimersByTime(1000);\n\n// Mocking a function that throws\nmockFn.mockImplementation(() => { throw new Error('fail'); });\n\n// Partial mocking (preserve some methods)\njest.mock('../db', () => {\n  const original = jest.requireActual('../db');\n  return { ...original, save: jest.fn() };\n});\n\n// Mocking constructors\njest.mock('../User');\nimport User from '../User';\nconst mockUserInstance = { getName: jest.fn().mockReturnValue('Mock') };\n(User as jest.Mock).mockImplementation(() => mockUserInstance);\n\n// Verify mock calls order\nexpect(mockFn).toHaveBeenNthCalledWith(1, 'first');\nexpect(mockFn).toHaveBeenNthCalledWith(2, 'second');",
  },
  {
    id: "concept-security-headers",
    type: "concept",
    topic: "Security",
    title: "Security Headers (CSP, HSTS, X-Frame-Options)",
    prompt:
      "What are security headers like CSP, HSTS, and X-Frame-Options? Why are they important?",
    expected:
      "CSP (Content Security Policy) restricts resources the browser can load; HSTS (HTTP Strict Transport Security) forces HTTPS; X-Frame-Options prevents clickjacking. They mitigate XSS, data injection, and man-in-the-middle attacks.",
    keywords: [
      "CSP",
      "HSTS",
      "X-Frame-Options",
    ...
    explanation:
      "Security headers are HTTP response headers that instruct browsers to enforce specific security policies. They are critical for protecting web applications against common attacks.\n\n**Content Security Policy (CSP):**\n- Prevents XSS and data injection attacks by whitelisting allowed sources for scripts, styles, images, etc.\n- Uses directives like `default-src`, `script-src`, `style-src`, `img-src`, `connect-src`, `font-src`.\n- Can report violations via `report-uri` or `report-to`.\n- Strict CSP avoids `unsafe-inline` and `unsafe-eval` where possible.\n- Example: `Content-Security-Policy: default-src 'self'; script-src 'self' https://trusted-cdn.com`\n\n**HTTP Strict Transport Security (HSTS):**\n- Forces browsers to always use HTTPS for the domain, preventing SSL stripping attacks.\n- Includes `max-age` (seconds), optional `includeSubDomains`, and `preload` (for HSTS preload list).\n- Example: `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload`\n\n**X-Frame-Options:**\n- Prevents clickjacking by controlling whether the page can be embedded in a frame/iframe.\n- Values: `DENY` (never allow), `SAMEORIGIN` (only same origin), `ALLOW-FROM uri` (deprecated; use CSP `frame-ancestors` instead).\n- Example: `X-Frame-Options: SAMEORIGIN`\n\n**Other important security headers:**\n- `X-Content-Type-Options: nosniff` – prevents MIME type sniffing.\n- `Referrer-Policy` – controls referrer information.\n- `Permissions-Policy` – controls browser features (camera, microphone, geolocation).\n\n**Interview tip:** Understand that security headers are defense in depth. Be able to configure a basic CSP for a modern React/Next.js app (avoid `unsafe-eval` and `unsafe-inline` except for hashes or nonces). Know the difference between `X-Frame-Options` and CSP `frame-ancestors` (CSP supersedes it).",
    code: "// Express.js setting security headers\nconst helmet = require('helmet');\napp.use(helmet()); // sets common headers: X-Content-Type-Options, X-Frame-Options, etc.\n\n// Manual CSP header (Express)\napp.use((req, res, next) => {\n  res.setHeader(\n    'Content-Security-Policy',\n    \"default-src 'self'; \" +\n    \"script-src 'self' https://trusted-cdn.com; \" +\n    \"style-src 'self' 'unsafe-inline'; \" +\n    \"img-src 'self' data: https://images.example.com; \" +\n    \"font-src 'self'; \" +\n    \"connect-src 'self' https://api.example.com; \" +\n    \"frame-ancestors 'none'; \" +\n    \"report-uri /csp-report\"\n  );\n  next();\n});\n\n// HSTS header\nres.setHeader('Strict-Transport-Security', 'max-age=31536000; includeSubDomains; preload');\n\n// X-Frame-Options\nres.setHeader('X-Frame-Options', 'DENY');\n\n// Additional headers\nres.setHeader('X-Content-Type-Options', 'nosniff');\nres.setHeader('Referrer-Policy', 'strict-origin-when-cross-origin');\n\n// Next.js: custom headers in next.config.js\nmodule.exports = {\n  async headers() {\n    return [\n      {\n        source: '/(.*)',\n        headers: [\n          { key: 'Content-Security-Policy', value: \"script-src 'self' ...\" },\n          { key: 'Strict-Transport-Security', value: 'max-age=31536000' },\n        ],\n      },\n    ];\n  },\n};\n\n// CSP with nonce (for inline scripts)\nconst crypto = require('crypto');\nconst nonce = crypto.randomBytes(16).toString('base64');\nres.setHeader('Content-Security-Policy', `script-src 'nonce-${nonce}' 'strict-dynamic'`);\n\n// HTML: <script nonce=\"...\">alert('safe')</script>\n\n// Reporting CSP violations (report-to)\nres.setHeader(\n  'Content-Security-Policy',\n  \"default-src 'self'; report-uri /csp-violation\"\n);\n\n// Nginx configuration\n// add_header Content-Security-Policy \"default-src 'self'\";\n// add_header Strict-Transport-Security \"max-age=31536000\";\n\n// Testing security headers with curl\n// curl -I https://example.com | grep -i \"content-security-policy\"\n",
  },
  {
    id: "concept-resource-hints",
    type: "concept",
    topic: "Performance",
    title: "Resource Hints (preload, prefetch, preconnect)",
    prompt:
      "What are preload, prefetch, and preconnect? How do they improve performance?",
    expected:
      "preload fetches critical resources early; prefetch fetches resources for future navigations; preconnect establishes early connection to an origin. They reduce latency by loading resources before they are needed.",
    keywords: [
      "preload",
      "prefetch",
      "preconnect",
    ...
    explanation:
      'Resource hints are `link` tags that tell the browser about resources or origins that will be needed soon, allowing the browser to perform actions early, reducing perceived latency.\n\n**preload:**\n- Fetches a resource that is critical for the current page and should be loaded as early as possible.\n- Use for fonts, hero images, critical CSS, JavaScript bundles.\n- Browser stores the resource in the memory cache and applies it when referenced.\n- `as` attribute is required to set proper priority (e.g., `as="style"`, `as="script"`, `as="image"`, `as="font"`).\n- Use `crossorigin` for CORS resources (e.g., fonts).\n\n**prefetch:**\n- Fetches resources that will be needed for future navigations (e.g., next page).\n- Browser fetches them at idle time and stores in the HTTP cache.\n- Use for next page resources, non‑critical assets.\n- Less aggressive than preload; lower priority.\n\n**preconnect:**\n- Performs DNS resolution, TCP handshake, and TLS negotiation (if HTTPS) for an origin.\n- Useful when you know you will make requests to a third‑party origin (e.g., CDN, API, analytics).\n- Saves time when the connection is later used.\n\n**Other resource hints:**\n- `dns-prefetch` – only DNS resolution, older alternative to preconnect.\n- `prerender` (deprecated) – loads entire page in hidden tab.\n\n**Comparison:**\n| Hint | Action | Priority | Use case |\n|------|--------|----------|----------|\n| preload | Fetch and cache | High | Current page critical resources |\n| prefetch | Fetch at idle time | Low | Future navigation |\n| preconnect | DNS, TCP, TLS | Moderate | Third‑party origins |\n\n**Interview tip:** Understand that `preload` is for same‑page resources, `prefetch` for future pages. Avoid overusing `preload` (it can compete with other critical resources). Use `fetchpriority` attribute for images (`high`, `low`, `auto`).',
    code: '<!-- preload critical font -->\n<link rel="preload" href="fonts/opensans.woff2" as="font" type="font/woff2" crossorigin>\n\n<!-- preload hero image -->\n<link rel="preload" href="hero.jpg" as="image" fetchpriority="high">\n\n<!-- preload CSS (not needed if referenced in head) -->\n<link rel="preload" href="styles.css" as="style" onload="this.onload=null;this.rel=\'stylesheet\'">\n\n<!-- preload JavaScript -->\n<link rel="preload" href="critical.js" as="script">\n\n<!-- prefetch next page resources -->\n<link rel="prefetch" href="product-page.html" as="document">\n<link rel="prefetch" href="product-images.webp" as="image">\n\n<!-- preconnect to third‑party origins -->\n<link rel="preconnect" href="https://api.example.com">\n<link rel="preconnect" href="https://fonts.googleapis.com">\n\n<!-- dns-prefetch (older, fallback for preconnect) -->\n<link rel="dns-prefetch" href="https://analytics.example.com">\n\n<!-- In Next.js, use next/head -->\n<Head>\n  <link rel="preload" href="/fonts/custom.woff2" as="font" crossOrigin="anonymous" />\n  <link rel="preconnect" href="https://api.example.com" />\n</Head>\n\n<!-- In React (react-helmet) -->\nimport { Helmet } from \'react-helmet\';\n<Helmet>\n  <link rel="preload" href="/critical.css" as="style" />\n</Helmet>\n\n<!-- Webpack dynamic import prefetch (React) -->\nimport(/* webpackPrefetch: true */ \'./module\');\n\n<!-- Webpack preload -->\nimport(/* webpackPreload: true */ \'./critical-module\');\n\n<!-- Using fetchpriority for images -->\n<img src="hero.jpg" fetchpriority="high">\n<img src="lazy.jpg" fetchpriority="low" loading="lazy">\n\n<!-- Preload with media (conditional) -->\n<link rel="preload" href="small.jpg" as="image" media="(max-width: 600px)">\n<link rel="preload" href="large.jpg" as="image" media="(min-width: 601px)">\n\n<!-- Note: Preload only resources that will be used within 3-5 seconds, otherwise waste bandwidth -->\n',
  },
  {
    id: "concept-tree-shaking",
    type: "concept",
    topic: "Performance",
    title: "Tree Shaking and Bundle Analysis",
    prompt:
      "What is tree shaking? How do bundle analysis tools help optimise JavaScript bundles?",
    expected:
      "Tree shaking is dead‑code elimination that removes unused exports from modules during bundling. It relies on ES module static structure. Bundle analysis tools (webpack‑bundle‑analyzer, rollup‑plugin‑visualizer) visualise bundle composition to identify large dependencies.",
    keywords: [
      "tree shaking",
    ...
    explanation:
      "Tree shaking is a term used by module bundlers (Webpack, Rollup, Parcel) to describe the removal of unused code from the final bundle. It relies on the static nature of ES6 `import`/`export` syntax.\n\n**How it works:**\n- The bundler builds a dependency graph starting from entry points.\n- It marks which exports are actually imported (used).\n- Unused exports (dead code) are omitted from the final bundle.\n\n**Prerequisites for tree shaking:**\n- Use ES modules (`import`/`export`) not CommonJS (`require`/`module.exports`).\n- The bundler must be configured to use ES modules (Webpack 4+ does by default for .mjs or `sideEffects` flag).\n- Mark packages with `\"sideEffects\": false` in `package.json` to tell the bundler that modules without side effects can be safely tree‑shaken.\n- Avoid importing from barrel files (`index.js` that re‑export many modules) – they can defeat tree shaking.\n\n**Bundle analysis tools:**\n- **webpack‑bundle‑analyzer:** Visualises Webpack bundle contents as an interactive treemap.\n- **rollup‑plugin‑visualizer:** Similar for Rollup.\n- **Vite** includes built‑in analysis (`rollup-plugin-visualizer`).\n- **Next.js** has `@next/bundle-analyzer`.\n\n**Common tree shaking pitfalls:**\n- Transpiling ES modules to CommonJS (Babel `@babel/preset-env` with `modules: 'commonjs'`).\n- Importing from libraries that are not ES module compatible or have side effects.\n- Using `require()` or dynamic imports that are less optimised.\n- CSS/SCSS imports have side effects (unless marked).\n\n**Interview tip:** Understand that tree shaking is not magic; it works best when libraries are authored as ES modules with `sideEffects: false`. Modern libraries like Lodash (in ES version) and date‑fns support tree shaking. Know how to configure Webpack to enable tree shaking and how to analyse bundles to find bloat.",
    code: "// package.json – sideEffects flag\n{\n  \"name\": \"my-lib\",\n  \"sideEffects\": false  // or [\"*.css\", \"*.scss\"]\n}\n\n// Webpack configuration for tree shaking\nmodule.exports = {\n  mode: 'production', // enables optimisations including tree shaking\n  optimization: {\n    usedExports: true, // marks unused exports\n    sideEffects: true  // respects sideEffects flag\n  }\n};\n\n// Babel preset causing issues (don't do this)\n// { 'presets': [['@babel/preset-env', { 'modules': 'commonjs' }]] }\n// Instead, use modules: false\n\n// Barrel file (index.js) that re‑exports – can reduce tree shaking effectiveness\nexport { a } from './a.js';\nexport { b } from './b.js';\n// Prefer direct imports to avoid pulling unused modules\n\n// Tree shaking friendly import\nimport { debounce } from 'lodash-es'; // good – only debounce\nimport _ from 'lodash'; // bad – whole library\n\n// Webpack Bundle Analyzer (Webpack)\nconst BundleAnalyzerPlugin = require('webpack-bundle-analyzer').BundleAnalyzerPlugin;\nmodule.exports = {\n  plugins: [\n    new BundleAnalyzerPlugin()\n  ]\n};\n\n// Next.js bundle analysis\n// npm install @next/bundle-analyzer\n// next.config.js\nconst withBundleAnalyzer = require('@next/bundle-analyzer')({\n  enabled: process.env.ANALYZE === 'true',\n});\nmodule.exports = withBundleAnalyzer({});\n\n// Vite bundle analysis (rollup plugin)\n// npm install rollup-plugin-visualizer\n// vite.config.js\nimport { visualizer } from 'rollup-plugin-visualizer';\nexport default {\n  plugins: [visualizer()]\n};\n\n// Analysing bundle manually: Webpack stats.json\n// webpack --profile --json > stats.json\n// npx webpack-bundle-analyzer stats.json\n\n// Tree shaking with dynamic imports – still works, but dynamic imports become separate chunks\nimport('./module').then(m => m.hi()); // only hi is included if module has sideEffects:false\n\n// Removing unused CSS with PurgeCSS (Tailwind uses it)\n// postcss.config.js\nmodule.exports = {\n  plugins: [\n    require('@tailwindcss/postcss'),\n    require('autoprefixer'),\n    process.env.NODE_ENV === 'production' && require('@fullhuman/postcss-purgecss')({\n      content: ['./src/**/*.html', './src/**/*.jsx', './src/**/*.tsx'],\n      defaultExtractor: content => content.match(/[\\w-/:]+(?<!:)/g) || []\n    })\n  ]\n};\n\n// Check if a module is tree‑shakeable: look for .mjs or \"module\" field in package.json\n// npm info lodash-es --json | grep module",
  },
  {
    id: "concept-database-indexing-basics",
    type: "concept",
    topic: "Database",
    title: "Database Indexing (B-tree, Hash, Bitmap) Basics",
    prompt:
      "What are database indexes? Explain B‑tree, hash, and bitmap indexes and when to use each.",
    expected:
      "Indexes speed up data retrieval. B‑tree indexes (default) support equality and range queries. Hash indexes support only equality (fast). Bitmap indexes are good for low‑cardinality columns in data warehousing (multiple predicates).",
    keywords: [
      "B-tree",
      "hash index",
      "bitmap index",
    ...
    explanation:
      "Database indexes are data structures that improve query performance by reducing the number of disk pages read. Different types serve different query patterns.\n\n**B‑tree (balanced tree):**\n- Default index type in most relational databases (PostgreSQL, MySQL, SQL Server).\n- Maintains sorted key order; supports equality (`=`), range (`>`, `<`, `BETWEEN`), and `LIKE 'prefix%'` queries.\n- Good for high‑cardinality columns.\n- Balanced structure ensures O(log n) lookups.\n- Used for primary keys, unique constraints, foreign keys.\n\n**Hash index:**\n- Uses a hash table; very fast for exact equality (`=`), but does not support range queries or sorting.\n- In PostgreSQL, hash indexes were historically not crash‑safe, but improved in v10+.\n- Good for columns with high cardinality when only equality needed.\n- Often not the first choice because B‑tree is already fast for equality and more versatile.\n\n**Bitmap index:**\n- Stores a bitmap for each distinct value (each bit corresponds to a row).\n- Efficient for low‑cardinality columns (e.g., gender, status, region).\n- Can combine multiple bitmaps with AND/OR for complex conditions.\n- Mostly used in data warehousing (Oracle, some others); PostgreSQL can simulate with `pg_trgm` or use extension?\n- Not available in all databases by default; PostgreSQL uses bitmap heap scan (not index type).\n\n**Other index types:**\n- **GiST:** Geometric data, full‑text search.\n- **GIN:** Inverted indexes for JSONB, arrays, full‑text.\n- **BRIN:** Block range index for very large tables with natural ordering (time‑series).\n- **Covering index:** Includes extra columns for index‑only scans.\n- **Partial index:** Index only a subset of rows.\n\n**Interview tip:** Understand trade‑offs: indexes speed reads but slow writes. Choose index type based on query patterns (equality vs range). B‑tree is the default; learn when to use others (GIN for JSONB, etc.). Be able to explain why hash indexes are rarely used.",
    code: "-- B‑tree index (default)\nCREATE INDEX idx_users_email ON users(email);\nCREATE INDEX idx_orders_customer_date ON orders(customer_id, order_date);\n\n-- Hash index (PostgreSQL)\nCREATE INDEX idx_users_id_hash ON users USING HASH (id);\n\n-- Bitmap index (not natively in PostgreSQL; but BRIN is somewhat similar)\n-- In Oracle: CREATE BITMAP INDEX idx_sales_status ON sales(status);\n\n-- PostgreSQL: Using a BRIN index for large time‑series (approximate bitmap)\nCREATE INDEX idx_sensor_time ON sensor_data USING BRIN (timestamp);\n\n-- GIN index for JSONB (inverted index)\nCREATE INDEX idx_products_attrs ON products USING GIN (attributes);\n\n-- Partial index (on active users)\nCREATE INDEX idx_active_users ON users(email) WHERE active = true;\n\n-- Expression index\nCREATE INDEX idx_users_lower_email ON users(LOWER(email));\n\n-- Covering index (PostgreSQL 11+)\nCREATE INDEX idx_orders_covering ON orders(customer_id) INCLUDE (order_date, total);\n\n-- Checking index type\nSELECT indexname, indexdef FROM pg_indexes WHERE tablename = 'users';\n\n-- When to use different types:\n-- B‑tree: Most queries, equality and range.\n-- Hash: Only equality, when you never need range or ordering.\n-- Bitmap: Low cardinality, multiple predicates in data warehouse.\n-- GIN: JSONB, array containment, full‑text search.\n-- GiST: Geometric, range overlap, full‑text (as alternative to GIN).\n-- BRIN: Huge tables with natural order (time, sequence), very size efficient.\n\n-- Performance test: EXPLAIN ANALYZE to see which index is used\nEXPLAIN (ANALYZE, BUFFERS) SELECT * FROM users WHERE email = 'alice@example.com';\n\n-- Dropping index\nDROP INDEX idx_users_email;\n\n-- Index statistics\nSELECT schemaname, tablename, indexname, idx_scan, idx_tup_read, idx_tup_fetch\nFROM pg_stat_user_indexes;\n\n-- Index bloat check (approximate)\nSELECT schemaname, tablename, indexname, pg_size_pretty(pg_relation_size(indexrelid)) AS size\nFROM pg_stat_user_indexes\nORDER BY pg_relation_size(indexrelid) DESC;",
  },
  {
    ...
    prompt:
      "What are common load balancing algorithms? Explain round robin, least connections, and IP hash.",
    expected:
      "Round robin distributes requests cyclically; least connections sends to the server with fewest active connections; IP hash maps a client IP to a specific server for session persistence (sticky sessions).",
    keywords: [
      "load balancing",
      "round robin",
      "least connections",
    ...
    explanation:
      "Load balancers distribute incoming traffic across multiple backend servers to improve scalability, availability, and reliability. Different algorithms serve different needs.\n\n**Round Robin:**\n- Simplest algorithm: distributes requests cyclically in order (server A, B, C, then back to A).\n- Works well when servers have roughly equal capacity and request loads are similar.\n- `Weighted Round Robin` allows assigning different weights for servers of different capacities.\n- Does not account for current load or server response time.\n\n**Least Connections:**\n- Sends the next request to the server with the fewest active connections.\n- Better when requests have variable processing times (some long, some short).\n- More adaptive than round robin.\n- `Weighted Least Connections` combines with server weights.\n\n**IP Hash (Consistent Hashing):**\n- Uses a hash of the client IP address (or other parts of request) to select a server.\n- Ensures that requests from the same client always go to the same server (sticky sessions).\n- Useful when session data is stored locally on each server (in‑memory sessions).\n- Also used in distributed caches (Redis, Memcached).\n- With `consistent hashing`, the effect of adding/removing servers is minimised.\n\n**Other algorithms:**\n- **Random:** Randomly selects a server; can cause uneven distribution.\n- **Least Response Time:** Sends to server with lowest response time (often used with health checks).\n- **Weighted algorithms:** Combine with server capacity.\n\n**Interview tip:** Understand that load balancing algorithms can be categorised into stateless (round robin, random) and state‑aware (least connections, least response time). IP hash is used for session stickiness but can be less balanced if many clients share a single IP (e.g., NAT). For modern containers, client‑side load balancing (e.g., gRPC, client‑side Ribbon) and service mesh (Istio) use similar algorithms.",
    code: '// Nginx configuration examples\n\n// Round Robin (default)\nupstream backend {\n  server backend1.example.com weight=3;\n  server backend2.example.com;\n  server backend3.example.com;\n}\n\n// Least Connections\nupstream backend {\n  least_conn;\n  server backend1.example.com;\n  server backend2.example.com;\n}\n\n// IP Hash (sticky sessions)\nupstream backend {\n  ip_hash;\n  server backend1.example.com;\n  server backend2.example.com;\n}\n\n// Generic proxy pass\nserver {\n  location / {\n    proxy_pass http://backend;\n  }\n}\n\n// HAProxy configuration\n// Round robin\nbackend round_robin\n    balance roundrobin\n    server web1 192.168.1.1:80 check\n    server web2 192.168.1.2:80 check\n\n// Least connections\nbackend least_conn\n    balance leastconn\n    server web1 192.168.1.1:80 check\n    server web2 192.168.1.2:80 check\n\n// Hash (source IP)\nbackend hash\n    balance source\n    hash-type consistent\n    server web1 192.168.1.1:80 check\n    server web2 192.168.1.2:80 check\n\n// Client‑side load balancing (gRPC round robin)\n// In Go client\nconn, err := grpc.Dial(\n    "dns:///my-service:50051",\n    grpc.WithDefaultServiceConfig(`{"loadBalancingPolicy":"round_robin"}`),\n    grpc.WithInsecure(),\n)\n\n// Kubernetes Service (kube-proxy) uses iptables/ipvs with random or round robin\n// By default, kube-proxy uses round robin (userspace) or random (iptables).\n// Can change to IPVS mode with scheduling algorithm (rr, lc, sh, etc.)\n\n// Consistent hashing in application code (pseudo)\nfunc GetServer(key string, servers []string) string {\n  hash := fnv.New32a()\n  hash.Write([]byte(key))\n  idx := hash.Sum32() % uint32(len(servers))\n  return servers[idx]\n}\n\n// With consistent hashing (ring)\n// Library "consistent" or implement hash ring\n\n// Testing load balancing behaviour: use curl repeatedly\n// for i in {1..10}; do curl -s http://lb.example.com/ ; done\n',
  },
];
