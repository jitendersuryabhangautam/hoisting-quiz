...
func main() {
  ...
  },
  {
    ...
    explanation:
      "ACID describes the guarantees a database provides when several statements must behave like one reliable unit of work. Atomicity means either the whole transaction commits or none of it does, which prevents half-completed business actions such as debiting one account without crediting another. Consistency means the transaction preserves rules enforced by the database, including constraints, foreign keys, and trigger logic. Isolation controls how concurrent transactions observe each other; stronger isolation prevents anomalies such as non-repeatable reads or write skew, but it usually increases locking, retries, or both. Durability means committed data survives crashes through mechanisms such as PostgreSQL's write-ahead log (WAL) and crash recovery. The interview nuance is that ACID is not free: stronger guarantees usually trade throughput and latency for correctness. In PostgreSQL, MVCC allows readers and writers to coexist efficiently, but serializable workloads can still need retry handling in the application. **Cross-references**: See 'Managing Database Transactions' for application code patterns, 'Transaction Isolation Levels' for anomaly trade-offs, and 'MVCC (Multi-Version Concurrency Control)' for PostgreSQL internals. **Operational tip**: Start from the business invariant you must protect, then choose the weakest isolation level that still preserves it.",
...
ROLLBACK;`,
  },
  {
    ...
    explanation:
      "An index is an auxiliary data structure that helps PostgreSQL find rows without scanning the entire table. The default B-tree index is ideal for equality, prefix, and range predicates; other index types exist because not every access pattern behaves like a sorted lookup. The read benefit can be dramatic, but indexes are not free: every INSERT, UPDATE, and DELETE must maintain index entries, which adds write amplification, storage cost, and vacuum work. Good indexing starts from actual query patterns, not from a generic rule like 'index every foreign key and filter column'. Composite indexes follow the leftmost-prefix rule, so column order should match the filters and sorting your workload really uses. Partial indexes help when only a subset of rows is queried often, and covering indexes (`INCLUDE`) can enable index-only scans. Common mistakes are indexing low-cardinality columns blindly, keeping duplicate indexes, and forgetting that unused indexes still slow writes. **Cross-references**: See 'EXPLAIN and Query Optimization' for verifying planner choices, 'PostgreSQL Index Types' for GIN/GiST/BRIN trade-offs, and 'Vacuum and Autovacuum' for index bloat maintenance. **Best practice**: Add an index only when a measured query pattern justifies its read benefit against its write cost.",
...
EXPLAIN ANALYZE SELECT * FROM users WHERE email = 'alice@example.com';`,
  },
  {
    ...
    explanation:
      "JSONB stores JSON in a decomposed binary format, which makes reads and containment queries much faster than the plain `json` type and enables indexing. It is useful when part of the schema is flexible or user-defined, such as product attributes, event payloads, preferences, or third-party webhook data. The trade-off is that JSONB is not a free replacement for relational modeling: updates often rewrite the whole document, constraints are weaker unless you add explicit checks, and ad hoc nested queries can become harder to optimize than normal columns. A practical pattern is hybrid modeling: keep stable, frequently filtered fields as regular columns, and store optional or sparse attributes in JSONB. Use GIN indexes for containment/existence queries and expression indexes when you repeatedly filter on a specific key such as `(attributes->>'brand')`. **Cross-references**: See 'Database Indexing' for GIN and expression index strategy, 'PostgreSQL Index Types' for operator-class choices, and 'Full-Text Search' when documents also need search capabilities. **Best practice**: Use JSONB to absorb schema variability, not to avoid schema design entirely.",
...
-- Expression index for a hot key lookup
...
FROM monthly_revenue;`,
  },
  {
    ...
    explanation:
      "Common Table Expressions (CTEs) are temporary named result sets defined within a SQL statement using the WITH clause, allowing for more readable and modular query construction. They can be referenced multiple times within the main query and can reference other CTEs, enabling complex query decomposition. Recursive CTEs (using WITH RECURSIVE) are particularly powerful for traversing hierarchical or tree-structured data, such as organizational charts, bill of materials, or graph structures. CTEs improve code maintainability by breaking down complex queries into logical steps and can sometimes provide performance benefits through optimization opportunities. While CTEs are generally materialized in PostgreSQL, they help organize query logic and support advanced patterns like recursive queries. They are essential for handling self-referential data and complex analytical queries that would be difficult or impossible with standard SQL constructs. **Cross-references**: See 'Window Functions' for combining analytics with CTEs, 'EXPLAIN and Query Optimization' for performance analysis, and 'SQL Joins' for complex query patterns. **Performance note**: CTEs can help with query readability but may not always be as efficient as equivalent subqueries - always test with EXPLAIN.",
    code: `-- Simple CTE
...
func GetInstance() *MyType {
  ...
  },
  {
    ...
    explanation:
      "The embed directive, introduced in Go 1.16, allows embedding static files and directories directly into the compiled binary at build time. This eliminates the need to distribute separate asset files, simplifying deployment and ensuring assets are always available. Files can be embedded as string literals, byte slices, or as an embed.FS (read-only filesystem interface). Common use cases include embedding HTML templates, CSS/JavaScript assets, configuration files, SQL migration scripts, or any static resources. The embedded files become part of the binary, reducing the number of deployment artifacts and preventing issues with missing files. However, embedded files cannot be modified at runtime, so they're best suited for read-only assets. **Cross-references**: See 'Caching Strategies' for embedding static assets, 'API Versioning Strategies' for embedding API specs, and 'Full-Text Search' for embedding search dictionaries. **Performance impact**: Embedded files load instantly (no disk I/O) but increase binary size by file size. **Best practice**: Use embed.FS for serving static files and compress large assets before embedding.",
    code: `import _ "embed"
//go:embed index.html
...
go mod tidy`,
  },
  {
    ...
    explanation:
      "Go ships with a strong testing toolchain in the standard library, so most teams start with `testing` before adding assertion libraries. Unit tests live in `*_test.go` files, fixtures usually go in `testdata/`, and integration tests are commonly separated by package boundaries or build tags. Table-driven tests are idiomatic because they make edge cases cheap to add and easy to review. Subtests with `t.Run` give better failure output, and `t.Parallel()` is useful only when cases do not share mutable state. For HTTP handlers, `httptest` gives you in-memory request/response testing; for concurrent code, `go test -race` catches data races that ordinary unit tests miss. Benchmarks (`BenchmarkXxx`) and fuzz tests (`FuzzXxx`) extend the same toolchain for performance and robustness. **Cross-references**: See 'Dependency Injection in Go' for testable design, 'Interfaces and Implicit Implementation' for consumer-side mocks, and 'Context Package' for cancellation-aware tests. **Best practice**: Test observable behavior and contracts, not private implementation details, so refactors do not create brittle tests.",
    code: `// main_test.go
package main
import (
    "context"
...
func (s stubRepo) Load(_ context.Context, _ string) (string, error) {
...
func TestServiceGet(t *testing.T) {
    tests := []struct {
        ...
        {"success", stubRepo{value: "alice"}, "alice", false},
        {"repo error", stubRepo{err: errors.New("db down")}, "", true},
    }
...
func BenchmarkServiceGet(b *testing.B) {
    svc := NewService(stubRepo{value: "alice"})
    for i := 0; i < b.N; i++ {
        _, _ = svc.Get(context.Background(), "42")
    }
}`,
  },
  {
