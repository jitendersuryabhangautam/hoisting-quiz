...
export default function MCQSection({ quiz, isVisible, onToggle }) {
  ...
  const handleStudyViewChange = (nextView) => {
    if (nextView === studyView) {
      return;
    }
  ...
  const nextStudyCard = () => {
    if (studyCardIndex < studyQuestions.length - 1) {
      setStudyCardIndex((value) => value + 1);
    }
  };
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/50 backdrop-blur-sm p-2 sm:items-center sm:p-4">
        <div className="w-full max-w-lg overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-2xl sm:max-h-[92vh] dark:border-slate-700/80 dark:bg-slate-900">
          {/* Result header */}
          <div
            className={`relative px-6 pb-8 pt-6 text-center ${isGreat ? "bg-linear-to-br from-emerald-500 to-teal-500" : isOkay ? "bg-linear-to-br from-amber-500 to-orange-500" : "bg-linear-to-br from-rose-500 to-pink-500"}`}
          >
          ...
          <div className="px-6 py-5">
            ...
            <p className="text-right text-xs font-medium text-slate-400 dark:text-slate-500">
              {percentage}%
            </p>
            ...
            <div className="mt-5 flex flex-col gap-2 sm:flex-row">
              <button
                ...
                className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-linear-to-r from-amber-500 to-orange-500 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:shadow-md dark:from-amber-500 dark:to-orange-600"
              >
                <svg
                  viewBox="0 0 16 16"
                  className="h-4 w-4"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/50 backdrop-blur-sm p-2 sm:items-center sm:p-4">
      <div className="flex w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-2xl sm:max-h-[92vh] dark:border-slate-700/80 dark:bg-slate-900">
        {/* ── Header ─────────────────────────────────────────────────────── */}
        <div className="relative flex items-center justify-between border-b border-slate-100 bg-linear-to-r from-amber-500 to-orange-500 px-5 py-3.5 dark:border-slate-800 dark:from-amber-600 dark:to-orange-600">
          <div className="pointer-events-none absolute inset-0 overflow-hidden">
          ...
          {mode === MODE_QUIZ ? (
            <>
              ...
              <div className="mb-5 rounded-2xl border border-slate-100 bg-slate-50/80 px-4 py-4 dark:border-slate-800 dark:bg-slate-800/40">
                <p className="wrap-break-word text-base font-semibold leading-relaxed text-slate-900 sm:text-lg dark:text-slate-100">
                  {question.question}
                </p>
              </div>
              ...
              <div className="min-w-0 space-y-2.5">
                {question.options.map((option, index) => {
                  ...
                  let optionStyle =
                    "border-slate-200 bg-white hover:border-amber-300 hover:bg-amber-50/50 dark:border-slate-700 dark:bg-slate-800/60 dark:hover:border-amber-600/50 dark:hover:bg-amber-500/5";
                  let letterStyle =
                  ...
                  if (showAnswer) {
                    if (isCorrect) {
                      ...
                      optionStyle =
                        "border-slate-100 bg-white opacity-50 dark:border-slate-800 dark:bg-slate-800/40";
                      letterStyle =
                  ...
                  return (
                    <button
                      ...
                      className={`group flex w-full items-start gap-3 wrap-break-word rounded-xl border px-3.5 py-3 text-left text-sm transition-all duration-200 ${optionStyle}`}
                    >
                      <span
                        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-xs font-bold transition ${letterStyle}`}
                      >
                        {showAnswer && isCorrect ? (
                          ...
                          <svg
                            ...
                            fill="currentColor"
                          >
                            <path d="M5.28 4.22a.75.75 0 00-1.06 1.06L6.94 8 4.22 10.72a.75.75 0 101.06 1.06L8 9.06l2.72 2.72a.75.75 0 101.06-1.06L9.06 8l2.72-2.72a.75.75 0 00-1.06-1.06L8 6.94 5.28 4.22z" />
                          </svg>
                        ) : (
                          letter
                        )}
                      </span>
                      <span className="pt-0.5 text-slate-800 dark:text-slate-200">
                        {option}
                      </span>
                    </button>
                  );
                ...
                <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="border-t border-slate-200 bg-white px-4 py-2.5 dark:border-slate-700 dark:bg-slate-900">
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-linear-to-r from-amber-500 to-orange-500 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:shadow-md dark:from-amber-500 dark:to-orange-600"
                    >
                      ...
                      >
                        <path
                          fillRule="evenodd"
                          d="M4.22 11.78a.75.75 0 010-1.06L7.94 7 4.22 3.28a.75.75 0 011.06-1.06l4.25 4.25a.75.75 0 010 1.06l-4.25 4.25a.75.75 0 01-1.06 0z"
                          clipRule="evenodd"
                        />
                      </svg>
                    </button>
            ...
            <>
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <div className="flex rounded-xl bg-slate-200/70 p-1 dark:bg-slate-800">
                  <button
                    type="button"
                    onClick={() => handleStudyViewChange("list")}
                  ...
                  <button
                    type="button"
                    onClick={() => handleStudyViewChange("card")}
                    className={`rounded-lg px-3 py-1.5 text-[11px] font-bold transition sm:text-xs ${
                      studyView === "card"
                        ? "bg-white text-amber-600 shadow-sm dark:bg-slate-700 dark:text-amber-400"
                        : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300"
              ...
              {studyView === "list" ? (
                    ...
                    return (
                      ...
                      >
                        <div className="border-b border-slate-100 bg-slate-50/80 px-4 py-3 dark:border-slate-700/50 dark:bg-slate-800/50">
                          <p className="wrap-break-word text-sm font-semibold leading-relaxed text-slate-900 sm:text-base dark:text-slate-100">
                        ...
                        <div className="space-y-1.5 px-4 py-3">
                          {studyQuestion.options.map((option, index) => {
                            const isCorrect =
                              index === studyQuestion.correctAnswer;
                            const letter = String.fromCharCode(65 + index);
                            ...
                            return (
                              ...
                              >
                                ...
                                <span
                                  className={`wrap-break-word pt-0.5 ${isCorrect ? "font-medium text-emerald-800 dark:text-emerald-300" : "text-slate-700 dark:text-slate-300"}`}
                                >
                                  {option}
                                </span>
                              </div>
                ...
                <div className="rounded-2xl border border-slate-200 dark:border-slate-700">
                  <div className="border-b border-slate-100 bg-slate-50/70 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/40">
                    <p className="text-xs font-bold text-slate-500 dark:text-slate-400">
                      Card {studyCardIndex + 1} / {studyQuestions.length}
                    </p>
                  </div>
                  <div className="space-y-4 px-4 py-4">
                    <p className="wrap-break-word text-sm font-semibold leading-relaxed text-slate-900 sm:text-base dark:text-slate-100">
                      {studyQuestions[studyCardIndex]?.question}
                    </p>

                    <div className="space-y-1.5">
                      {studyQuestions[studyCardIndex]?.options.map(
                        (option, index) => {
                          const activeQuestion = studyQuestions[studyCardIndex];
                          const isCorrect =
                          ...
                          return (
                            <div
                              key={`${activeQuestion.question}-card-${index}`}
                              className={`flex items-start gap-2.5 rounded-xl border px-3 py-2.5 text-sm ${
                                isCorrect
                                  ? "border-emerald-300 bg-emerald-50 dark:border-emerald-600/40 dark:bg-emerald-500/10"
                                  : "border-slate-100 bg-white dark:border-slate-700/50 dark:bg-slate-800/30"
                    ...
                    <div className="rounded-xl border border-slate-100 bg-slate-50/50 px-3 py-3 dark:border-slate-700/50 dark:bg-slate-800/30">
                      <p className="mb-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                        Explanation
                      </p>
                      <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-400">
                        {studyQuestions[studyCardIndex]?.explanation}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between border-t border-slate-100 px-4 py-3 dark:border-slate-700">
                    <button
                      ...
                      className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                    >
                      Prev
                    </button>
                    <button
                      ...
                      disabled={studyCardIndex === studyQuestions.length - 1}
                      className="rounded-xl bg-linear-to-r from-amber-500 to-orange-500 px-3 py-2 text-xs font-bold text-white transition hover:shadow-md disabled:cursor-not-allowed disabled:opacity-40 dark:from-amber-500 dark:to-orange-600"
                    >
                      Next
                    </button>
                  </div>
