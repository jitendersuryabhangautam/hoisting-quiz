...
const TRACKS = [
  ...
  { name: "JS Output", icon: "JO", color: "from-yellow-400 to-amber-500", desc: "Predict output for hoisting, closures, async, and scope.", href: "/output" },
  { name: "JS Theory", icon: "JT", color: "from-cyan-400 to-sky-500", desc: "Concept-focused JavaScript interview questions and answers.", href: "/theory" },
  { name: "JS Implementation", icon: "JI", color: "from-slate-700 to-slate-900 dark:from-slate-300 dark:to-slate-100", darkText: true, desc: "Write solutions for core JavaScript coding tasks.", href: "/implementation" },
  { name: "Go Output", icon: "GO", color: "from-sky-400 to-blue-500", desc: "Practice Go output behavior and execution ordering.", href: "/go-output" },
  { name: "Backend Theory", icon: "BT", color: "from-indigo-400 to-violet-500", desc: "Backend system concepts across APIs, architecture, and design.", href: "/backend" },
  { name: "Backend Implementation", icon: "BI", color: "from-sky-300 to-cyan-500", desc: "Hands-on backend implementation question set.", href: "/backend-implementation" },
  { name: "DB Theory", icon: "DB", color: "from-blue-400 to-blue-600", desc: "Core database theory concepts and interview preparation.", href: "/db-theory" },
  { name: "PG Implementation", icon: "PG", color: "from-emerald-400 to-green-600", desc: "PostgreSQL schema and implementation-oriented practice.", href: "/postgresql-implementation" },
];
...
const ROADMAP = [
  { title: "Coding Playground", desc: "Inline code runner with instant feedback." },
  { title: "Spaced Repetition", desc: "Adaptive review for weak topics." },
  { title: "Mock Interviews", desc: "Timed mixed-question interview simulation." },
  { title: "Personal Study Plans", desc: "Track-based study sequence with progress." },
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-16 pt-16 sm:px-6 sm:pb-24 sm:pt-24 lg:pt-28">
          <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
            <div className="mx-auto max-w-2xl text-center lg:mx-0 lg:text-left">
              ...
              <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 sm:text-5xl lg:text-6xl dark:text-white">
                ...
                <span className="bg-brand-gradient bg-clip-text text-transparent"> Structured Practice</span>
              </h1>
              <p className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg lg:mx-0 dark:text-slate-400">
                Output, theory, backend, and implementation tracks in one place so you can revise fast and answer with confidence.
              </p>
              <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row lg:justify-start sm:justify-center">
                <Link href="/output" className="flex items-center gap-2 rounded-2xl bg-brand-gradient px-7 py-3.5 text-base font-bold text-white shadow-lg shadow-amber-500/25 transition-all hover:scale-105 hover:shadow-xl hover:shadow-amber-500/30 dark:shadow-amber-600/20">
                  Start Learning
                </Link>
                <a href="#features" className="flex items-center gap-1.5 rounded-2xl border-2 border-amber-400/60 bg-white px-6 py-3.5 text-sm font-semibold text-amber-700 transition hover:border-amber-500 hover:bg-amber-50 dark:border-amber-600/40 dark:bg-transparent dark:text-amber-400 dark:hover:border-amber-500 dark:hover:bg-amber-500/10">
                  See what is inside
                </a>
              </div>
            </div>
          ...
          <div className="mx-auto mt-14 grid max-w-lg grid-cols-3 gap-6 sm:mt-20 sm:max-w-xl">
            {[{ value: "8", label: "Tracks" }, { value: "300+", label: "Questions" }, { value: "100%", label: "Practice Focus" }].map((stat) => (
      ...
      <section ref={tracksRef} className={`border-t border-slate-100 bg-slate-50/80 py-16 sm:py-24 dark:border-slate-800 dark:bg-slate-900/50 transition-all duration-700 ${tracksVis ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-900 sm:text-3xl dark:text-white">Interview pages by purpose.</h2>
          </div>
          <div className="mx-auto mt-10 grid max-w-5xl gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {TRACKS.map((t) => (
              <Link key={t.name} href={t.href} className="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-slate-700/80 dark:bg-slate-900">
      ...
      <section id="features" ref={featRef} className={`py-16 sm:py-24 transition-all duration-700 ${featVis ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-xs font-bold uppercase tracking-widest text-amber-600 dark:text-amber-400">What You Get</p>
            <h2 className="mt-2 text-2xl font-bold text-slate-900 sm:text-3xl dark:text-white">Everything you need to ace interviews</h2>
          </div>
          <div className="mx-auto mt-10 grid max-w-5xl gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map((f) => (
              <div key={f.title} className="group relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-slate-700/80 dark:bg-slate-900">
                <div className="pointer-events-none absolute -bottom-2 -right-2 h-24 w-24 rounded-full bg-amber-100/50 dark:bg-amber-500/10" aria-hidden="true" />
      ...
      <section ref={roadRef} className={`border-t border-violet-100 bg-gradient-to-b from-violet-50/60 via-slate-50/80 to-slate-50/80 py-16 sm:py-24 dark:border-violet-900/30 dark:from-violet-950/20 dark:via-slate-900/50 dark:to-slate-900/50 transition-all duration-700 ${roadVis ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-2xl font-bold text-slate-900 sm:text-3xl dark:text-white">What is coming next</h2>
          </div>
          <div className="mx-auto mt-10 grid max-w-5xl gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {ROADMAP.map((r) => (
              <div key={r.title} className="group relative overflow-hidden rounded-2xl border border-dashed border-violet-300/70 bg-white/70 p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-violet-400/80 dark:border-violet-700/50 dark:bg-slate-900/70 dark:hover:border-violet-600/70">
